        ...
        <div>
          ...
          whs.map(w=>`<div class="card u-fdf33f23" >
            ...
            <div style="background:var(--bg-1);border:1px solid var(--border);border-radius:6px;padding:5px 9px;font-size:11px;font-family:monospace;color:var(--accent-text);margin-bottom:8px;display:flex;align-items:center;gap:6px">
              ...
              <button data-act-click="hCopyWebhookUrl(${jsArg(w.id)})" aria-label="Copy webhook URL" title="Copy webhook URL" class="icon-btn" style="width:26px;height:26px;font-size:13px;flex-shrink:0">📋</button>
            </div>
            <div style="display:flex;gap:6px;align-items:center">
              <button data-act-click="testWebhook(${jsArg(w.id)})" class="btn btn-ghost btn-sm">▶ Test</button>
              <button data-act-click="deleteWebhook(${jsArg(w.id)})" aria-label="Delete webhook" title="Delete webhook" class="icon-btn" style="width:26px;height:26px;font-size:13px;margin-left:auto">🗑</button>
            </div>
          </div>`).join('')}
        </div>
        <div>
