    ...
    <div style="background:rgba(247,118,142,0.08);border:1px solid var(--danger);border-radius:10px;padding:12px 16px;margin-bottom:16px">
      ...
      ${(alerts.alerts||[]).map(a=>`
        <div style="display:flex;align-items:center;gap:8px;font-size:11px;margin-bottom:4px">
          ...
          <button class="btn-sm" aria-label="Resolve alert" title="Resolve alert" data-act-click="finopsResolveAlert(${jsArg(a.id)})" style="margin-left:auto;font-size:10px">✓</button>
        </div>`).join('')}
    </div>`:''}
...
async function finopsCreateCap() {
  ...
  if (!name?.trim()) return;
  const scope    = await gmChoose('Scope', 'What does this cap apply to?',
    ['agent','goal','platform'], 'agent');
  if (scope === null) return;
  const limitIn  = await gmPrompt('Limit ($USD):', '1.00');
  if (limitIn === null) return;
  const limitUsd = limitIn.trim() || '1.00';
  const period   = await gmChoose('Period', 'How often does the cap reset?',
    ['hour','day','week'], 'day');
