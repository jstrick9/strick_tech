// Agentic OS — Connect Hub
// ───────────────────────────────────────────────────────────────────────────
// ONE pane over every integration source. Before this, CONNECT was five panes
// over three registries that did not know about each other (23 MCP tools,
// 6 gateway servers, 53 connector rows of which 45 were test residue), each
// with its own vocabulary for "is this usable?".
//
// The question a user actually has is singular: "what can my agent do right
// now, and what do I need to set up?" That is what this pane answers, in the
// shape ChatGPT, Claude and Manus all converge on — ready things first, setup
// instructions attached to the thing that needs them, and a Test button that
// distinguishes *configured* from *actually working*.
...
function connectRenderBody() {
  ...
  // The setup prompt is shown FIRST and only when there is something to do.
...
// Guided credential entry: asks for exactly the fields the provider needs,
// named as the provider names them. Previously this information lived only
// inside the error message of a failed call.
...
// "Configured" and "working" are different things. A connector row saying
// active only means credentials were stored, not that they are valid.
