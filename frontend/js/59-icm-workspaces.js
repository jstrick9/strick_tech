/*
 * Agentic OS — ICM Workspaces pane (frontend/js/59-icm-workspaces.js)
 *
 * "The folder is the app." This pane makes that literal: the workspace file
 * tree IS the interface, markdown is rendered and editable in place, and stage
 * status is derived from what is actually on disk rather than tracked
 * separately (a second copy of state is a copy that drifts).
 *
 * The Routing tab is the important one. Van Clief's named #1 failure of ICM at
 * scale is that the agent starts in the wrong folder and nobody can see that it
 * did -- the layered context silently never loads and the run still looks fine.
 * So entry routing is surfaced here as a visible, testable, logged decision:
 * what enters where, why this request matched, and what was decided before.
 */
(function () {
  'use strict';
  ...
  // ── folders: the tree IS the interface ──────────────────────────────────
  function stageBar(w) {
  ...
  async function renderWalkTest() {
    ...
    try {
      ...
      // A refusal has to say how to lift it, or people route around the gate.
      el.innerHTML = `
  ...
  async function icmwsSave() {
    ...
    try {
      ...
      // A save can repair OR break the walk test; either way the verdict the
      // user is looking at is now stale.
      renderWalkTest();
      // Routes live in CONTEXT.md, so saving it can change routing. Reload the
      // table rather than letting the sidebar show a stale answer.
      await loadWorkspaces();
      renderTree();
    } catch (e) {
  ...
  // ── templates: method and instance live apart ───────────────────────────
  // A template is a workspace with its L4 products removed. Instantiating is a
  // folder copy, not a schema render, so what you see in the template is
  // exactly what the new workspace gets.
  ...
  async function icmwsExtractTemplate() {
    ...
    try {
      const d = await api('/api/icm/templates/extract', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        // Send an explicit template_id derived from the NAME the user typed.
        // Without it the id falls back to the workspace id, so two templates
        // extracted from one workspace collide however they are named — the
        // user renames, retries, and gets the same refusal.
        body: JSON.stringify({
      ...
      // renderIcmTemplates() rebuilds this whole tab, which DESTROYS the note
      // element the message was just written into. Setting it before the
      // re-render made success silent while failure (which does not re-render)
      // showed fine. Re-render first, then write into the fresh node.
  ...
  // ── routing: the decision, made visible ─────────────────────────────────
  ...
  // ── audit: restructure mode + system map ────────────────────────────────
  // Reads are free; the migration is gated. The Apply button is deliberately
  // not the primary action -- the plan is what the user is meant to read.
  let auditPlan = null;
  ...
  // The delegated dispatcher resolves data-act-click by plain property lookup
  // on window, and this file is IIFE-wrapped, so every handler must be exported
  // explicitly or the buttons silently do nothing.
  ...
  // Navigation dispatches through MASTER_PANE_REGISTRY in 00-pane-registry.js,
  // where 'icm' resolves window.renderWorkspacesIcmPane. Note PANE_RENDERERS
  // (written by 12-information-hierarchy.js) is NOT the nav hook -- nothing
  // reads it -- so registering there alone renders nothing.
