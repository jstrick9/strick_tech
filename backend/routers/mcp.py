...
import asyncio, os, json, subprocess, shutil, hashlib, time
...
ALLOWED_CMDS = {"ls", "cat", "echo", "pwd", "grep", "find", "wc", "head", "tail",
                "node", "npm", "npx", "pip", "git"}  # python3 removed — use code.run tool instead
...
def list_tools():
    ...
    return {
        "tools": [
            {"name": name, "description": info["desc"], "args": info["args"]}
...
async def call_tool(req: Request):
    ...
    try:
        ...
        return {"ok": False, "tool": tool, "error": f"Internal error: {e}", "duration_ms": round((time.time()-t0)*1000)}
...
async def agent_with_tools(req: Request):
    ...
    tool_docs = "\n".join(
        ...
        for name, info in TOOLS.items() if name in allowed
    )
    ...
    try:
        ...
        for node in _ast.walk(tree):
            ...
            if isinstance(node, (_ast.Import, _ast.ImportFrom)):
                raise ToolError(f"Import statements not allowed. Use math, json, datetime, re.")
