...
import json, logging, uuid, asyncio
...
BUILTIN_CONNECTORS = [
    {"connector_id":"conn_slack","name":"Slack","description":"Send messages, manage channels, look up users in Slack workspaces","category":"communication","icon":"💬","status":"unconfigured","auth_type":"api_key","capabilities":json.dumps(["send_message","list_channels","get_user","post_file"]),"config":json.dumps({"bot_token_env":"SLACK_BOT_TOKEN","default_channel":"general"})},
    {"connector_id":"conn_jira","name":"Jira","description":"Create issues, update status, query projects, add comments in Atlassian Jira","category":"project_mgmt","icon":"🎫","status":"unconfigured","auth_type":"basic","capabilities":json.dumps(["create_issue","update_issue","get_issue","list_projects","add_comment","search_issues"]),"config":json.dumps({"base_url_env":"JIRA_BASE_URL","email_env":"JIRA_EMAIL","token_env":"JIRA_API_TOKEN"})},
    {"connector_id":"conn_gdrive","name":"Google Workspace","description":"Create Docs, read/write Sheets, create Calendar events via Google APIs","category":"productivity","icon":"📊","status":"unconfigured","auth_type":"oauth","capabilities":json.dumps(["create_doc","read_sheet","write_sheet","create_calendar_event","list_events"]),"config":json.dumps({"client_id_env":"GOOGLE_CLIENT_ID","client_secret_env":"GOOGLE_CLIENT_SECRET","token_env":"GOOGLE_OAUTH_TOKEN"})},
    {"connector_id":"conn_email","name":"Email (Multi-Provider)","description":"Send email via SMTP (any provider), Gmail OAuth2, or Outlook OAuth2. Supports multi-account, HTML, CC/BCC, attachments, and templates.","category":"communication","icon":"📧","status":"unconfigured","auth_type":"multi","capabilities":json.dumps(["send_email","send_html_email","test_connection","list_accounts"]),"config":json.dumps({"modes":["smtp","gmail_oauth","outlook_oauth"],"smtp":{"host_env":"SMTP_HOST","port_env":"SMTP_PORT","user_env":"SMTP_USER","pass_env":"SMTP_PASS","from_env":"SMTP_FROM"},"gmail":{"token_env":"GMAIL_OAUTH_TOKEN","from_env":"GMAIL_FROM"},"outlook":{"token_env":"OUTLOOK_OAUTH_TOKEN","from_env":"OUTLOOK_FROM"}})},
    {"connector_id":"conn_github","name":"GitHub","description":"Create issues, PRs, read files, trigger workflows via GitHub REST API","category":"devops","icon":"🐙","status":"unconfigured","auth_type":"api_key","capabilities":json.dumps(["create_issue","create_pr","get_file","trigger_workflow","list_repos"]),"config":json.dumps({"token_env":"GITHUB_TOKEN"})},
    {"connector_id":"conn_webhook","name":"Outbound Webhook","description":"POST arbitrary JSON payloads to any external HTTP endpoint","category":"integration","icon":"🪝","status":"active","auth_type":"none","capabilities":json.dumps(["post_webhook","post_with_auth"]),"config":json.dumps({})},
    {"connector_id":"conn_notion","name":"Notion","description":"Create pages, update databases, read content from Notion workspaces","category":"productivity","icon":"📝","status":"unconfigured","auth_type":"api_key","capabilities":json.dumps(["create_page","update_page","query_database","get_page"]),"config":json.dumps({"token_env":"NOTION_TOKEN"})},
    {"connector_id":"conn_salesforce","name":"Salesforce CRM","description":"Query contacts, leads, opportunities, create/update records in Salesforce","category":"crm","icon":"☁️","status":"unconfigured","auth_type":"oauth","capabilities":json.dumps(["query_records","create_record","update_record","get_contact","create_lead"]),"config":json.dumps({"instance_url_env":"SALESFORCE_INSTANCE_URL","token_env":"SALESFORCE_ACCESS_TOKEN"})},
]
...
def _ensure_schema():
    ...
    try:
        ...
        for c in BUILTIN_CONNECTORS:
            existing = con.execute("SELECT connector_id FROM connector_registry WHERE connector_id=?", (c["connector_id"],)).fetchone()
            if not existing:
                con.execute("""INSERT INTO connector_registry (connector_id,name,description,category,icon,status,auth_type,capabilities,config,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
                    (c["connector_id"],c["name"],c["description"],c["category"],c["icon"],c["status"],c["auth_type"],c["capabilities"],c["config"],now,now))
        con.commit()
    finally:
        con.close()
...
def _connector_dict(row) -> dict:
    ...
    for f in ("capabilities","config","credentials"):
        try: d[f] = json.loads(d.get(f,"[]" if f=="capabilities" else "{}"))
        except: pass
    # Mask credentials
...
async def _exec_slack(action: str, payload: dict, creds: dict) -> dict:
    ...
    async def _get(path: str, params: dict | None = None) -> dict:
        async with httpx.AsyncClient(timeout=15) as cl:
            r = await cl.get(f"https://slack.com/api/{path}",
                             headers=headers, params=params or {})
            return r.json()

    async def _post(path: str, body: dict) -> dict:
        async with httpx.AsyncClient(timeout=15) as cl:
            r = await cl.post(f"https://slack.com/api/{path}",
    ...
    if action == "send_rich_message":
        ...
        if payload.get("blocks"):       body["blocks"]      = payload["blocks"]
        if payload.get("attachments"):  body["attachments"] = payload["attachments"]
        if payload.get("thread_ts"):    body["thread_ts"]   = payload["thread_ts"]
    ...
    if action == "create_channel":
        ...
        d = await _post("conversations.create", {
            "name":       name,
    ...
    if action == "invite_to_channel":
        ...
        d = await _post("conversations.invite", {
            "channel": payload.get("channel",""),
    ...
    if action == "list_members":
        ...
        d = await _get("conversations.members", {
            "channel": payload.get("channel",""), "limit": 200
        })
        return _slack_ok(d, {"members": d.get("members",[]),
    ...
    if action == "post_file":
        ...
        async with httpx.AsyncClient(timeout=20) as cl:
            ...
            await cl.post(upload_url, content=raw_bytes,
        ...
        if d3.get("ok"):
            ...
            return {"ok": True, "file_id": file_id,
                    ...
                    "url": f.get("permalink",""), "channel": channel}
    ...
    if action == "get_message":
        ...
        d = await _get("conversations.history", {
            "channel":  payload.get("channel",""),
            "latest":   payload.get("ts",""),
            "limit":    1,
            "inclusive": True
        })
        if not d.get("ok"):
        ...
        m = msgs[0]
        return {"ok": True, "ts": m.get("ts"), "text": m.get("text",""),
                "user": m.get("user",""), "type": m.get("type","")}

    # ── update_message ────────────────────────────────────────────────────────
    if action == "update_message":
        """payload: { "channel": "C01ABC123", "ts": "1234567890.123", "text": "updated text" }"""
        d = await _post("chat.update", {
            "channel": payload.get("channel",""),
            "ts":      payload.get("ts",""),
            "text":    payload.get("text","")
        })
    ...
    if action == "set_topic":
        ...
        d = await _post("conversations.setTopic", {
            "channel": payload.get("channel",""),
    ...
    return {"ok": False, "error": f"Unknown Slack action: '{action}'. Valid: "
...
async def _exec_jira(action: str, payload: dict, creds: dict) -> dict:
    ...
    def _ok(status: int, d: dict, extra: dict | None = None) -> dict:
        ...
        if not success:
            ...
            errs = d.get("errors", {})
            return {"ok": False,
                    "error": "; ".join(msgs) if msgs else str(errs) or f"HTTP {status}",
                    "status": status}
        result = {"ok": True}
        result.update(extra or {})
        return result
    ...
    if action == "list_projects":
        status, d = await _get(f"{API3}/project/search", {
            "maxResults": payload.get("max_results", 50),
            "orderBy":    "name",
        })
        if status != 200: return _ok(status, d)
        projects = [{
            "key":         p.get("key"),
            "name":        p.get("name"),
            "id":          p.get("id"),
    ...
    if action == "get_project":
        ...
        if not key: return {"ok":False,"error":"payload.project_key required"}
    ...
    if action == "search_issues":
        ...
        issues = [_fmt_issue(i) for i in d.get("issues",[])]
        return {"ok": True, "issues": issues, "count": len(issues),
                "total": d.get("total", len(issues))}

    # ── get_issue ─────────────────────────────────────────────────────────────
    if action == "get_issue":
        """payload: { "issue_key":"KAN-1" }"""
        key = payload.get("issue_key","") or payload.get("key","")
        if not key: return {"ok":False,"error":"payload.issue_key required (e.g. 'KAN-1')"}
    ...
    if action == "update_issue":
        ...
        key = payload.get("issue_key","") or payload.get("key","")
        if not key: return {"ok":False,"error":"payload.issue_key required"}
        fields: dict = {}
    ...
    if action == "delete_issue":
        ...
        return {"ok": status == 204,
                "issue_key": key,
                "error": "" if status == 204 else f"HTTP {status}"}
    ...
    if action == "add_comment":
        ...
        if not key or not body: return {"ok":False,"error":"payload.issue_key and body required"}
        status, d = await _post(f"{API3}/issue/{key}/comment",
            {"body": _adf(body)})
        return _ok(status, d, {
            "comment_id": d.get("id"),
            "issue_key":  key,
    ...
    if action == "get_comments":
        ...
        key = payload.get("issue_key","") or payload.get("key","")
        if not key: return {"ok":False,"error":"payload.issue_key required"}
        status, d = await _get(f"{API3}/issue/{key}/comment",
            {"maxResults": payload.get("max_results",20)})
        if status != 200: return _ok(status, d)
        comments = [{
            "id":      c.get("id"),
    ...
    if action == "transition_issue":
        ...
        status, d = await _post(f"{API3}/issue/{key}/transitions",
    ...
    if action == "assign_issue":
        ...
        if not key: return {"ok":False,"error":"payload.issue_key required"}
        async with httpx.AsyncClient(timeout=15) as cl:
            r = await cl.put(f"{API3}/issue/{key}/assignee",
                headers=HDR, json={"accountId": account_id})
        return {"ok": r.status_code == 204,
                "issue_key": key,
    ...
    if action == "get_issue_types":
        ...
        if project:
            ...
            if numeric_id:
                ...
                if status == 200 and isinstance(d, list):
                    return {"ok": True, "issue_types": [{"id":t.get("id"),"name":t.get("name"),
                            "subtask":t.get("subtask",False)} for t in d]}
        else:
        ...
        return {"ok": True,
                "issue_types": [{"id":t.get("id"),"name":t.get("name"),
                                  "subtask":t.get("subtask",False)} for t in types]}
    ...
    if action == "search_users":
        ...
        if not query: return {"ok":False,"error":"payload.query required"}
        status, d = await _get(f"{API3}/user/search",
            {"query": query, "maxResults": payload.get("max_results",10)})
        if status != 200: return _ok(status, d)
        users = [{"account_id":u.get("accountId"),"display_name":u.get("displayName"),
                  "email":u.get("emailAddress",""),"active":u.get("active")}
    ...
    if action == "list_boards":
        ...
        if payload.get("project_key"): params["projectKeyOrId"] = payload["project_key"]
        if payload.get("type"):        params["type"]           = payload["type"]
        status, d = await _get(f"{AGILE}/board", params)
        if status != 200: return _ok(status, d)
        boards = [{"id":b.get("id"),"name":b.get("name"),"type":b.get("type"),
    ...
    if action == "get_board":
        ...
        if not bid: return {"ok":False,"error":"payload.board_id required"}
    ...
    if action == "list_sprints":
        ...
        if not bid: return {"ok":False,"error":"payload.board_id required"}
        params: dict = {"maxResults": payload.get("max_results",20)}
        if payload.get("state"): params["state"] = payload["state"]
        ...
        if status == 400 and "does not support sprints" in str(d):
            ...
            return {"ok": True, "sprints": [], "count": 0,
                    "note": "This board is Kanban type — sprints are only available on Scrum boards"}
        if status != 200: return _ok(status, d)
        sprints = [{"id":s.get("id"),"name":s.get("name"),"state":s.get("state"),
    ...
    if action == "create_sprint":
        ...
        if payload.get("start_date"): body["startDate"] = payload["start_date"] + "T00:00:00.000Z"
        if payload.get("end_date"):   body["endDate"]   = payload["end_date"]   + "T00:00:00.000Z"
        if payload.get("goal"):       body["goal"]      = payload["goal"]
...
def _build_mime_message(frm: str, to: str, subject: str, body: str,
                         cc: str = "", bcc: str = "",
                         reply_to: str = "", is_html: bool = False,
                         attachments: list | None = None) -> "email.mime.multipart.MIMEMultipart":
...
async def _smtp_send(host: str, port: int, user: str, pw: str,
                     frm: str, recipients: list[str], raw_msg: str,
...
async def _gmail_oauth_send(access_token: str, frm: str, recipients: list[str],
                             raw_msg: str) -> dict:
...
async def _outlook_oauth_send(access_token: str, frm: str, to: str,
                               subject: str, body: str, is_html: bool = False) -> dict:
...
async def _exec_email(action: str, payload: dict, creds: dict) -> dict:
    ...
    if cc:  recipients += [r.strip() for r in cc.split(",") if r.strip()]
    if bcc: recipients += [r.strip() for r in bcc.split(",") if r.strip()]
    ...
    if action == "list_accounts":
        ...
        if os.getenv("GMAIL_OAUTH_TOKEN") or os.getenv("GOOGLE_OAUTH_TOKEN"):
            accounts.append({"id":"gmail_oauth","provider":"gmail","email":os.getenv("GMAIL_FROM",""),"mode":"gmail_oauth"})
        if os.getenv("OUTLOOK_OAUTH_TOKEN"):
            accounts.append({"id":"outlook_oauth","provider":"outlook","email":os.getenv("OUTLOOK_FROM",""),"mode":"outlook_oauth"})
        return {"ok": True, "accounts": accounts, "count": len(accounts)}
    ...
    if action == "test_connection":
        if mode == "smtp":
            ...
            try:
                ...
                return {"ok": True, "mode": "smtp", "host": host, "port": port, "user": user,
            ...
            if not token: return {"ok": False, "error": "GMAIL_OAUTH_TOKEN not set"}
            import httpx
            async with httpx.AsyncClient(timeout=5) as c:
                r = await c.get("https://www.googleapis.com/oauth2/v1/userinfo",
                                headers={"Authorization": f"Bearer {token}"})
            if r.status_code == 200:
            ...
            if not token: return {"ok": False, "error": "OUTLOOK_OAUTH_TOKEN not set"}
            import httpx
            async with httpx.AsyncClient(timeout=5) as c:
            ...
            if r.status_code == 200:
                d = r.json()
                return {"ok": True, "mode":"outlook_oauth",
    ...
    if action not in ("send_email","send_html_email","send"):
        return {"ok": False, "error": f"Unknown action '{action}'. Use: send_email, send_html_email, test_connection, list_accounts"}
    ...
    if mode == "gmail_oauth":
        ...
        if not token:
            return {"ok": False, "error": "Outlook OAuth: access_token required in creds or OUTLOOK_OAUTH_TOKEN env var"}
...
async def _exec_github(action: str, payload: dict, creds: dict) -> dict:
    ...
    def _ok(status: int, d: dict, extra: dict | None = None) -> dict:
        success = 200 <= status < 300
        if not success:
            return {"ok": False, "error": d.get("message", f"HTTP {status}"),
    ...
    if action == "list_accounts":
        ...
        if env_tok:
            accounts.append({"id": "github_env", "source": "env",
                             "token_prefix": env_tok[:12] + "..."})
        if token and token != env_tok:
            accounts.append({"id": "github_inline", "source": "configured",
    ...
    if action == "list_repos":
        ...
        status, d = await _get("/user/repos", {
    ...
    if action == "get_repo":
        ...
        owner = payload.get("owner", ""); repo = payload.get("repo", "")
    ...
    if action == "create_repo":
        ...
        if not name: return {"ok": False, "error": "payload.name required"}
        status, d = await _post("/user/repos", {
            "name":        name,
            "description": payload.get("description",""),
            "private":     payload.get("private", False),
            "auto_init":   payload.get("auto_init", True),
        })
        return _ok(status, d, {"repo":   d.get("full_name"),
                                "url":   d.get("html_url"),
                                "clone": d.get("clone_url","")})

    # ── list_issues ───────────────────────────────────────────────────────────
    if action == "list_issues":
        """payload: { "owner":"user","repo":"name","state":"open","per_page":20 }"""
        owner = payload.get("owner",""); repo = payload.get("repo","")
        if not owner or not repo: return {"ok":False,"error":"payload.owner and payload.repo required"}
        status, d = await _get(f"/repos/{owner}/{repo}/issues", {
        ...
        } for i in (d if isinstance(d,list) else []) if "pull_request" not in i]
        return {"ok": True, "issues": issues, "count": len(issues)}
    ...
    if action == "create_issue":
        ...
        body = {"title": title}
        if payload.get("body"):      body["body"]      = payload["body"]
        if payload.get("labels"):    body["labels"]    = payload["labels"]
        if payload.get("assignees"): body["assignees"] = payload["assignees"]
        status, d = await _post(f"/repos/{owner}/{repo}/issues", body)
        return _ok(status, d, {"issue_number": d.get("number"),
                                "title":        d.get("title"),
                                "url":          d.get("html_url"),
    ...
    if action == "update_issue":
        ...
        for key in ("title","body","state","labels","assignees"):
            if payload.get(key) is not None: body[key] = payload[key]
    ...
    if action == "close_issue":
        ...
        if not all([owner,repo,number]):
            return {"ok":False,"error":"payload.owner, repo, issue_number required"}
        status, d = await _patch(f"/repos/{owner}/{repo}/issues/{number}",
    ...
    if action == "list_prs":
        ...
        owner = payload.get("owner",""); repo = payload.get("repo","")
        if not owner or not repo: return {"ok":False,"error":"payload.owner and repo required"}
        status, d = await _get(f"/repos/{owner}/{repo}/pulls",
            {"state": payload.get("state","open"), "per_page": payload.get("per_page",20)})
        if status != 200: return _ok(status, d)
        prs = [{"number":p.get("number"),"title":p.get("title"),
                "state":p.get("state"),"url":p.get("html_url"),
                "user":p.get("user",{}).get("login",""),
                "head":p.get("head",{}).get("ref",""),
                "base":p.get("base",{}).get("ref",""),
                "draft":p.get("draft",False)} for p in (d if isinstance(d,list) else [])]
    ...
    if action == "create_pr":
        ...
        return _ok(status, d, {"pr_number": d.get("number"),
                                "title":     d.get("title"),
                                "url":       d.get("html_url"),
                                "state":     d.get("state","open")})
    ...
    if action == "get_file":
        ...
        params = {}
        if payload.get("ref"): params["ref"] = payload["ref"]
        status, d = await _get(f"/repos/{owner}/{repo}/contents/{path}", params)
        if status != 200: return _ok(status, d)
        import base64
        content_b64 = d.get("content","").replace("\n","")
        try: content = base64.b64decode(content_b64).decode("utf-8", errors="replace")
    ...
    if action == "create_file":
        ...
        content_b64 = base64.b64encode(
            payload.get("content","").encode("utf-8")).decode()
        body = {
            "message": payload.get("message", f"Add {path} via Agentic OS"),
            "content": content_b64,
        }
        if payload.get("branch"): body["branch"] = payload["branch"]
        if payload.get("sha"):    body["sha"]    = payload["sha"]
        status, d = await _put(f"/repos/{owner}/{repo}/contents/{path}", body)
        cf = d.get("content",{})
        return _ok(status, d, {"path":   cf.get("path"),
                                "sha":    cf.get("sha"),
                                "url":    cf.get("html_url",""),
    ...
    if action == "list_branches":
        ...
        owner = payload.get("owner",""); repo = payload.get("repo","")
        if not owner or not repo: return {"ok":False,"error":"payload.owner and repo required"}
        status, d = await _get(f"/repos/{owner}/{repo}/branches",
            {"per_page": payload.get("per_page",30)})
        if status != 200: return _ok(status, d)
        branches = [{"name":b.get("name"),
    ...
    if action == "create_branch":
        ...
        if status != 200: return _ok(status, src)
        sha    = src.get("object",{}).get("sha","")
        status, d = await _post(f"/repos/{owner}/{repo}/git/refs",
            {"ref": f"refs/heads/{branch}", "sha": sha})
        return _ok(status, d, {"branch": branch,
                                "from":   from_br,
                                "sha":    sha[:7],
                                "ref":    d.get("ref","")})
    ...
    if action == "get_commits":
        ...
        if payload.get("branch"): params["sha"] = payload["branch"]
    ...
    if action == "list_workflows":
        ...
        if status != 200: return _ok(status, d)
        wfs = [{"id":w.get("id"),"name":w.get("name"),"state":w.get("state"),
                "path":w.get("path")} for w in d.get("workflows",[])]
    ...
    if action == "list_workflow_runs":
        ...
        path = (f"/repos/{owner}/{repo}/actions/workflows/{wf}/runs"
                if wf else f"/repos/{owner}/{repo}/actions/runs")
    ...
    if action == "list_gists":
        ...
        if status != 200: return _ok(status, d)
        gists = [{"id":g.get("id"),"description":g.get("description",""),
                  "public":g.get("public"),"url":g.get("html_url"),
    ...
    if action == "search_code":
        ...
        q = payload.get("q","")
        if not q: return {"ok":False,"error":"payload.q (search query) required"}
        status, d = await _get("/search/code",
            {"q": q, "per_page": payload.get("per_page",10)})
        if status != 200: return _ok(status, d)
        items = [{"name":i.get("name"),"path":i.get("path"),
                  "repo":i.get("repository",{}).get("full_name",""),
                  "url":i.get("html_url","")} for i in d.get("items",[])]
    ...
    if action == "search_repos":
        ...
        q = payload.get("q","")
        if not q: return {"ok":False,"error":"payload.q (search query) required"}
        status, d = await _get("/search/repositories", {
            "q":        q,
            "sort":     payload.get("sort","stars"),
            "per_page": payload.get("per_page",10),
        })
        if status != 200: return _ok(status, d)
        repos = [{"name":r.get("full_name"),"stars":r.get("stargazers_count"),
                  "language":r.get("language"),"description":r.get("description","")[:80],
                  "url":r.get("html_url")} for r in d.get("items",[])]
    ...
    if action == "get_user":
        ...
        username = payload.get("username","")
        if not username: return {"ok":False,"error":"payload.username required"}
    ...
    if action == "star_repo":
        ...
        async with httpx.AsyncClient(timeout=15, follow_redirects=True) as cl:
            r = await cl.put(f"{BASE}/user/starred/{owner}/{repo}",
                headers={**HDR,"Content-Length":"0"}, content=b"")
        return {"ok": r.status_code == 204,
                "repo":  f"{owner}/{repo}",
                "error": "" if r.status_code == 204 else f"HTTP {r.status_code}"}
    ...
    if action == "list_starred":
        """payload: { "per_page":10 }"""
        status, d = await _get("/user/starred",
            {"per_page": payload.get("per_page",10)})
        if status != 200: return _ok(status, d)
        repos = [{"name":r.get("full_name"),"stars":r.get("stargazers_count"),
...
async def _exec_gdrive(action: str, payload: dict, creds: dict) -> dict:
    ...
    async def _refresh() -> str:
        ...
        async with httpx.AsyncClient(timeout=10) as cl:
            r = await cl.post("https://oauth2.googleapis.com/token", data={
    ...
    if action == "list_accounts":
        accounts = []
        if access_token:
            accounts.append({"id":"google_access_token","type":"access_token",
                             "token_prefix":access_token[:20]+"..."})
        if refresh_tok:
            accounts.append({"id":"google_refresh_token","type":"refresh_token",
                             "token_prefix":refresh_tok[:20]+"..."})
        return {"ok": True, "accounts": accounts, "count": len(accounts)}
    ...
    if action == "gmail_list_messages":
        ...
        if payload.get("query"):     params["q"]        = payload["query"]
        if payload.get("label"):     params["labelIds"] = payload["label"]
        ...
        for m in msgs[:payload.get("max_results",10)]:
            _, md = await _get(f"{GMAIL}/messages/{m['id']}",
        ...
        return {"ok": True, "messages": result_msgs, "count": len(result_msgs),
    ...
    if action == "gmail_get_message":
        ...
        if not mid: return {"ok":False,"error":"payload.message_id required"}
    ...
    if action == "gmail_send":
        ...
        if not to: return {"ok":False,"error":"payload.to required"}

        msg = MIMEMultipart("alternative") if is_html else MIMEText(body,"plain")
        msg["To"]      = to
        msg["Subject"] = subject
        if payload.get("cc"):       msg["Cc"]       = payload["cc"]
        if payload.get("bcc"):      msg["Bcc"]      = payload["bcc"]
        if payload.get("reply_to"): msg["Reply-To"] = payload["reply_to"]
        if is_html:
            import re
            plain = re.sub(r"<[^>]+>","",body).strip()
        ...
        return _ok(status, d, {"message_id": d.get("id"),
    ...
    if action == "gmail_create_draft":
        ...
        raw = base64.urlsafe_b64encode(msg.as_bytes()).decode()
        status, d = await _post(f"{GMAIL}/drafts",
            {"message": {"raw": raw}})
        if status == 403 and "insufficient" in str(d).lower():
            return {"ok": False,
                    ...
                    "status": 403}
        return _ok(status, d, {"draft_id": d.get("id"),
    ...
    if action == "gmail_search":
        ...
        q = payload.get("query","")
        if not q: return {"ok":False,"error":"payload.query required"}
        status, d = await _get(f"{GMAIL}/messages",
            {"q":q,"maxResults":payload.get("max_results",10)})
        if status != 200: return _ok(status, d)
        msgs = d.get("messages",[])
        return {"ok": True, "message_ids": [m["id"] for m in msgs],
                "count": len(msgs), "total_estimate": d.get("resultSizeEstimate",0),
                "query": q}
    ...
    if action == "drive_list_files":
        ...
        if payload.get("query"):     q_parts.append(payload["query"])
        if payload.get("folder_id"): q_parts.append(f"'{payload['folder_id']}' in parents")
        mime_map = {"document":"application/vnd.google-apps.document",
        ...
        status, d = await _get(f"{DRIVE}/files", {
            "q":        " AND ".join(q_parts),
    ...
    if action == "drive_get_file":
        ...
        if not fid: return {"ok":False,"error":"payload.file_id required"}
        status, d = await _get(f"{DRIVE}/files/{fid}",
    ...
    if action == "drive_create_folder":
        ...
        if payload.get("parent_id"): body["parents"] = [payload["parent_id"]]
        status, d = await _post(f"{DRIVE}/files", body)
        return _ok(status, d, {"folder_id":d.get("id"),"name":d.get("name"),
    ...
    if action == "drive_upload_text":
        ...
        return _ok(r.status_code, d, {"file_id":d.get("id"),"name":d.get("name"),
    ...
    if action == "drive_delete_file":
        ...
        return {"ok": status == 204, "file_id": fid,
                "error": "" if status == 204 else f"HTTP {status}"}
    ...
    if action == "docs_create":
        ...
        if payload.get("content") and doc_id:
            await _post(f"{DOCS}/{doc_id}:batchUpdate", {"requests":[{
                "insertText":{"location":{"index":1},"text":payload["content"]}
            }]})
        return {"ok": True,
                "doc_id":  doc_id,
                "title":   d.get("title"),
    ...
    if action == "docs_get":
        ...
        if not did: return {"ok":False,"error":"payload.doc_id required"}
    ...
    if action == "docs_append_text":
        ...
        did  = payload.get("doc_id","")
        text = payload.get("text","")
        if not did or not text: return {"ok":False,"error":"payload.doc_id and text required"}
        # Get doc to find end index
        _, doc = await _get(f"{DOCS}/{did}")
        content = doc.get("body",{}).get("content",[])
        end_idx = max((e.get("endIndex",1) for e in content), default=1) - 1
        status, d = await _post(f"{DOCS}/{did}:batchUpdate", {"requests":[{
            "insertText":{"location":{"index":end_idx},"text":"\n"+text}
        }]})
        return _ok(status, d, {"doc_id":did,"appended_chars":len(text),
    ...
    if action == "docs_replace_text":
        ...
        if not all([did,find]): return {"ok":False,"error":"payload.doc_id and find required"}
        status, d = await _post(f"{DOCS}/{did}:batchUpdate", {"requests":[{
            "replaceAllText":{
    ...
    if action == "sheets_create":
        ...
        body   = {
            ...
            "sheets": [{"properties":{"title":s}} for s in sheets]
        }
    ...
    if action == "sheets_get":
        ...
        if not sid: return {"ok":False,"error":"payload.spreadsheet_id required"}
    ...
    if action == "calendar_list":
        ...
        if status != 200: return _ok(status, d)
        cals = [{"id":c.get("id"),"name":c.get("summary"),
    ...
    if action == "calendar_list_events":
        ...
        if payload.get("time_max"):  params["timeMax"] = payload["time_max"]
        if payload.get("query"):     params["q"]       = payload["query"]
        ...
        return {"ok": True, "events": events, "count": len(events),
                "calendar": d.get("summary",""), "timezone": d.get("timeZone","")}
    ...
    if action == "calendar_create_event":
        ...
        if payload.get("description"): event_body["description"] = payload["description"]
        if payload.get("location"):    event_body["location"]    = payload["location"]
    ...
    if action == "calendar_delete_event":
        ...
        cal_id = payload.get("calendar_id","primary")
        if not eid: return {"ok":False,"error":"payload.event_id required"}
        status, _ = await _delete(f"{CAL}/calendars/{cal_id}/events/{eid}")
        return {"ok": status == 204, "event_id": eid,
                "error": "" if status == 204 else f"HTTP {status}"}
...
async def _exec_notion(action: str, payload: dict, creds: dict) -> dict:
    ...
    if action == "create_page":
        ...
        if not children and payload.get("content"):
            ...
            children = [{
                ...
                "type": "paragraph",
                "paragraph": {
                    "rich_text": _notion_rich_text(payload["content"])
                }
            }]
        if children:
    ...
    if action == "update_page":
        ...
        if r.status_code not in (200, 201):
            return {"ok": False, "error": r.text[:400], "status_code": r.status_code}
        data = r.json()
        return {"ok": True, "page_id": data.get("id"), "url": data.get("url"), "last_edited_time": data.get("last_edited_time")}
    ...
    if action == "append_page_content":
        ...
        if not children and payload.get("text"):
            children = [{
                "object": "block",
                "type": "paragraph",
                "paragraph": {"rich_text": _notion_rich_text(payload["text"])}
            }]
        if not children:
    ...
    if action == "list_comments":
        ...
        if r.status_code == 403:
            return {"ok": False, "error": "comments scope not granted — enable 'Read comments' in integration settings", "status_code": 403}
        r.raise_for_status()
        data = r.json()
    ...
    if action == "create_comment":
        ...
        if r.status_code == 403:
            return {"ok": False, "error": "comments scope not granted — enable 'Insert comments' in integration settings", "status_code": 403}
        if r.status_code not in (200, 201):
            return {"ok": False, "error": r.text[:400], "status_code": r.status_code}
        data = r.json()
...
async def execute_connector(connector_id: str, action: str, payload: dict,
                            agent_id: str = "system",
                            inline_creds: dict | None = None) -> dict:
    ...
    con = _get_conn()
    try:
        con.execute("""INSERT INTO connector_executions (exec_id,connector_id,action,agent_id,payload_hash,status,result,error,duration_ms,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)""",
            (exec_id, connector_id, action, agent_id, ph, status,
             json.dumps(result, default=str)[:2000], result.get("error","")[:500], duration_ms, _now()))
        con.execute("UPDATE connector_registry SET call_count=call_count+1, last_used=? WHERE connector_id=?", (_now(), connector_id))
        con.commit()
    finally:
        con.close()
    ...
    try:
        ...
        append_entry(agent_id, agent_id.title(), "connector_exec",
                     f"[{connector["name"]}] {action}: {str(payload)[:80]}",
                     authority="agent", risk_level="medium",
                     outcome="success" if result.get("ok") else "failure",
                     metadata={"exec_id": exec_id, "connector_id": connector_id,
...
def list_connectors(category: str="", status: str=""):
    ...
    sql = "SELECT * FROM connector_registry" + ((" WHERE " + " AND ".join(where)) if where else "") + " ORDER BY category, name"
    con = _get_conn()
    try:
...
async def register_connector(req: Request):
    ...
    con = _get_conn()
    try:
        con.execute("""INSERT INTO connector_registry (connector_id,name,description,category,icon,status,auth_type,capabilities,config,credentials,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
            (conn_id, name[:100], (body.get("description") or "")[:500],
             (body.get("category") or "custom")[:30], (body.get("icon") or "🔌")[:8],
             "unconfigured", (body.get("auth_type") or "none")[:20],
...
async def configure_connector(connector_id: str, req: Request):
    ...
    con = _get_conn()
    try:
        con.execute("UPDATE connector_registry SET credentials=?, config=?, status=?, updated_at=? WHERE connector_id=?",
...
async def run_connector(connector_id: str, req: Request):
    ...
    result = await execute_connector(connector_id, action, payload, agent_id,
...
async def test_connector(connector_id: str):
    ...
    con = _get_conn()
    try:
        row = con.execute("SELECT name, status, auth_type FROM connector_registry WHERE connector_id=?", (connector_id,)).fetchone()
    finally:
        con.close()
    if not row:
        return JSONResponse({"ok": False, "error": "Not found"}, status_code=404)
    return {
        ...
        "message": "Connector is active and ready" if row["status"]=="active" else f"Connector needs credentials ({row['auth_type']} auth)"
    }
...
def connector_stats():
    ...
    try:
        ...
        top_used    = con.execute("SELECT name,call_count FROM connector_registry ORDER BY call_count DESC LIMIT 5").fetchall()
    finally:
        con.close()
    return {
