...
def record_performance(agent_id: str, task_type: str = "general",
...
def leaderboard(limit: int = 20, days: int = 30, task_type: str = ""):
    ...
    try:
        ...
        rows = con.execute(f"""
            ...
            LIMIT :limit
        """, {"task_type": task_type, "limit": min(limit, 100)}).fetchall()
...
def list_policies(agent_id: str = ""):
    ...
    try:
        if agent_id:
            ...
            rows = con.execute(
                "SELECT * FROM agent_policies ORDER BY agent_id,policy_type"
            ).fetchall()
    finally:
        con.close()
...
async def update_policy(policy_id: int, req: Request):
    ...
    try:
        ...
        if "policy_rule" in body:
            ...
            if rule:
                sets.append("policy_rule=?"); vals.append(rule)
        if not sets:
...
def discover_agents():
    ...
    try:
        ...
        agents = []; activity = {}; loops = {}; policy_counts = {}
    finally:
        con.close()
