...
async def add_entity(req: Request):
    ...
    try:
        ...
        if existing:
            eid = existing['id']
            con.execute(
                'UPDATE kg_entities SET description=?,properties=?,confidence=?,updated_at=CURRENT_TIMESTAMP WHERE id=?',
                (
            ...
            con.execute(
                'INSERT INTO kg_entities(id,name,type,description,properties,source,confidence) VALUES (?,?,?,?,?,?,?)',
                (
                    eid,
                    name,
                    etype,
...
def search_entities(q: str = '', type: str = '', limit: int = 30):
    """Execute or process search entities operation."""
    from ..services.memory_db import get_conn
...
def get_entity(entity_id: str):
    """Retrieve and return get entity."""
    from ..services.memory_db import get_conn
    ...
    try:
        ...
        facts = con.execute(
            'SELECT * FROM kg_facts WHERE subject_id=? ORDER BY created_at DESC', (entity_id,)
        ).fetchall()
    finally:
        con.close()
    if not ent:
...
def traverse_graph(entity_id: str, depth: int = 2, relation: str = ''):
    ...
    try:
        while queue:
            ...
            rels = con.execute(
                f'SELECT * FROM kg_relations WHERE from_id=?{where} ORDER BY confidence DESC LIMIT 20', params
            ).fetchall()
            for r in rels:
                rd = dict(r)
                edges.append(rd)
...
Answer based on the graph data. If information isn't in the graph, say so."""
        ...
        result = await llm_svc.complete(
            [{'role': 'user', 'content': prompt}], agent_id='knowledge_graph', max_tokens=500, inject_steering=False
        )
...
@router.get('/stats')
def graph_stats():
    """Execute or process graph stats operation."""
    from ..services.memory_db import get_conn
    ...
    try:
        ...
        by_rel = con.execute(
            'SELECT relation, COUNT(*) as cnt FROM kg_relations GROUP BY relation ORDER BY cnt DESC LIMIT 10'
        ).fetchall()
    finally:
        con.close()
    return {
