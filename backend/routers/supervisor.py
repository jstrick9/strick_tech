...
SPECIALIST_SKILLS = {
    "researcher":   ["research", "search", "find", "gather", "analyze data", "survey", "literature", "web", "information"],
...
async def _decompose_goal(goal_text: str, goal_id: str, run_id: str) -> list[dict]:
    ...
    return [
        {"seq": 1, "title": "Research & Plan",
         "description": f"Research and plan how to achieve: {goal_text}",
         "agent_hint": "researcher", "depends_on": [], "risk_level": "low", "estimated_tokens": 600},
        {"seq": 2, "title": "Execute",
         "description": f"Execute the plan for: {goal_text}",
         "agent_hint": "builder",   "depends_on": [1], "risk_level": "low", "estimated_tokens": 800},
        {"seq": 3, "title": "Review & Finalize",
         "description": f"Review and finalize the output for: {goal_text}",
         "agent_hint": "reviewer",  "depends_on": [2], "risk_level": "low", "estimated_tokens": 400},
    ]
...
async def _execute_task(task: dict, run_id: str, goal_text: str,
                        context_so_far: list[dict]) -> dict:
    ...
    con = _get_conn()
    try:
        agent_row = con.execute("SELECT name, role, system_prompt FROM agents WHERE id=?",
                                (agent_id,)).fetchone()
    finally:
        con.close()
...
async def _synthesize(goal_text: str, completed_tasks: list[dict]) -> str:
    ...
    result = await complete(
        messages, agent_id="orchestrator",
...
def _topo_sort(tasks: list[dict]) -> list[list[dict]]:
    ...
    for _ in range(len(tasks) + 1):
        ...
        wave = [t for t in remaining
                if all(d in completed_seqs for d in (t.get("depends_on") or []))]
        if not wave:
...
def _update_run(run_id: str, **kwargs):
    ...
    try:
        ...
        con.execute(f"UPDATE supervisor_runs SET {sets} WHERE run_id=?",
...
def _update_task(task_id: str, **kwargs):
    ...
    try:
        sets = ", ".join(f"{k}=?" for k in kwargs)
        con.execute(f"UPDATE supervisor_tasks SET {sets} WHERE task_id=?",
...
def _is_killed(run_id: str) -> bool:
    con = _get_conn()
    try:
        row = con.execute("SELECT 1 FROM supervisor_kill_switches WHERE run_id=?",
                          (run_id,)).fetchone()
...
async def _run_supervisor(run_id: str, goal_id: str, goal_text: str):
    ...
    try:
        # ── Phase 1: Decompose ─────────────────────────────────────────────
        await broadcast({"type": "supervisor_update", "run_id": run_id,
                         "status": "decomposing", "message": "Brain is decomposing your goal…"})
        ...
        try:
            ...
            con.execute("UPDATE supervisor_runs SET task_count=?, status='scheduled', updated_at=? WHERE run_id=?",
                        (len(db_tasks), _now(), run_id))
            con.commit()
        finally:
            con.close()
        
        append_entry("supervisor", "Supervisor", "goal_decomposed",
        ...
        await broadcast({"type": "supervisor_update", "run_id": run_id,
                         ...
                         "tasks": [{"task_id": t["task_id"], "title": t["title"],
                                    "agent_id": t["agent_id"], "seq": t["seq"]} for t in db_tasks]})
        ...
        for wave_idx, wave in enumerate(waves):
            ...
            async def _run_wave_task(task: dict):
                ...
                await broadcast({"type": "supervisor_task_start", "run_id": run_id,
                                 "task_id": task["task_id"], "agent_id": task["agent_id"],
                                 "title": task["title"], "seq": task["seq"]})
                try:
                    ...
                    _update_task(task["task_id"],
                                 status="done", output=output[:5000],
                                 tokens=tokens, cost=cost,
                                 duration_ms=dur, completed_at=_now())
                    
                    task["output"]  = output
                    task["tokens"]  = tokens
                    task["cost"]    = cost
                    
                    append_entry(task["agent_id"], task["agent_id"].title(),
                                 "task_completed", f"[{task['title']}] {output[:150]}",
                                 reasoning=task["description"][:200],
                                 authority="supervisor", risk_level=task.get("risk_level","low"),
                                 outcome="success", metadata={"run_id": run_id,
                                                               "task_id": task["task_id"]})
                    await broadcast({"type": "supervisor_task_done", "run_id": run_id,
                    ...
                    append_entry(task["agent_id"], task["agent_id"].title(),
                                 "task_failed", f"[{task['title']}] {err}",
                                 authority="supervisor", risk_level="high",
                                 outcome="failure", metadata={"run_id": run_id})
        ...
        append_entry("supervisor", "Supervisor", "run_completed",
                     f"Goal '{goal_text[:80]}' done — score {eval_score:.0%}",
                     reasoning=eval_notes[:200],
                     authority="user", risk_level="low", outcome="success",
                     metadata={"run_id": run_id, "eval_score": eval_score,
                               "total_tokens": total_tokens, "duration_ms": duration_ms})
        
        await broadcast({"type": "supervisor_done", "run_id": run_id,
                         "eval_score": eval_score, "eval_notes": eval_notes,
                         "final_output": final_output[:500],
                         "duration_ms": duration_ms, "total_tokens": total_tokens})
        
        log.info("Supervisor run %s complete — score %.0f%% in %dms",
                 run_id, eval_score * 100, duration_ms)
    
    except Exception as e:
        err = str(e)[:500]
        log.error("Supervisor run %s crashed: %s", run_id, err)
        _update_run(run_id, status="failed",
                    final_output=f"Supervisor error: {err}",
                    duration_ms=_epoch_ms() - t_start)
        append_entry("supervisor", "Supervisor", "run_failed", err,
                     authority="user", risk_level="high", outcome="failure",
...
def get_run(run_id: str):
    ...
    try:
        ...
        tasks = con.execute(
            "SELECT * FROM supervisor_tasks WHERE run_id=? ORDER BY seq",
            (run_id,)
        ).fetchall()
    finally:
        con.close()
...
def list_runs(limit: int = 20, status: str = ""):
    ...
    try:
        if status:
            ...
            rows = con.execute(
                "SELECT * FROM supervisor_runs ORDER BY created_at DESC LIMIT ?",
                (min(limit, 100),)
            ).fetchall()
    finally:
        con.close()
...
async def kill_run(run_id: str, req: Request):
    ...
    try:
        ...
        con.execute("UPDATE supervisor_runs SET status='killed', updated_at=? WHERE run_id=?",
                    (_now(), run_id))
        con.commit()
    finally:
        con.close()
    ...
    from ..routers.audit_log import append_entry
    append_entry("user", "User", "supervisor_killed",
                 f"Run {run_id} killed: {reason}",
                 authority="user", risk_level="medium", outcome="blocked",
                 metadata={"run_id": run_id})
...
def supervisor_stats():
    ...
    try:
        ...
        by_status= con.execute(
            "SELECT status, COUNT(*) as cnt FROM supervisor_runs GROUP BY status"
        ).fetchall()
        avg_score= con.execute(
            "SELECT AVG(eval_score) FROM supervisor_runs WHERE eval_score IS NOT NULL"
        ).fetchone()[0]
...
def get_dag(run_id: str):
    ...
    for w in sorted(waves.keys()):
        ...
        wave_summary.append({
            "wave":   w,
            "count":  len(wt),
            "status": "done"    if all(t["status"] == "done" for t in wt)
                      else "running" if any(t["status"] == "running" for t in wt)
                      else "failed"  if any(t["status"] == "failed" for t in wt)
                      else "pending",
        })
        ...
        for _ in range(180):  # max 15 min at 5s poll
            ...
            if run:
                ...
                data  = json.dumps({
                    ...
                    "tasks": [{"seq": t["seq"], "title": t["title"],
                               "agent_id": t["agent_id"], "status": t["status"]} for t in tasks],
                }, default=str)
                yield f"data: {data}\n\n"
