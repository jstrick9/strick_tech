...
import asyncio, io, os, hashlib, logging, json
...
async def speak(req: Request):
    ...
    if use_cache and cache_file.exists():
        ...
        return StreamingResponse(
            _file_iter(cache_file), media_type="audio/mpeg",
    ...
    try:
        ...
        return StreamingResponse(
            iter([audio_bytes]), media_type="audio/mpeg",
            headers={
                "X-Cache":  "MISS",
                "X-Voice":  edge_voice,
...
async def speak_get(
    ...
    if cache_file.exists():
        return StreamingResponse(
            _file_iter(cache_file), media_type="audio/mpeg",
            headers={"X-Cache": "HIT", "X-Voice": edge_voice}
        )
    ...
    try:
        ...
        return StreamingResponse(
            iter([audio_bytes]), media_type="audio/mpeg",
            headers={"X-Cache": "MISS", "X-Voice": edge_voice}
        )
    except Exception as e:
        return JSONResponse({"ok": False, "error": str(e)}, status_code=500)
...
def list_voices():
    ...
    return {
        "voices":       [{"id": k, "name": v, "lang": v.split("-")[0]+"-"+v.split("-")[1]} for k, v in EDGE_VOICES.items()],
...
async def elevenlabs_speak(req: Request):
    ...
    if not api_key:
        return JSONResponse(
            {"ok": False, "error": "ELEVENLABS_API_KEY not set. Set it in Settings → API Keys."},
            status_code=503
        )
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if r.status_code == 200:
                ...
                return JSONResponse({"ok": False, "error": f"ElevenLabs error {r.status_code}: {r.text[:200]}"}, status_code=r.status_code)
    except Exception as ex:
