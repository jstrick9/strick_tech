...
ALWAYS_INTERRUPT = {
    "delete_file", "delete_database", "drop_table", "rm_rf",
    "send_email", "send_message", "post_to_social",
    "stripe_charge", "financial_transaction",
    "deploy_to_production", "push_to_main",
    "git_force_push", "secret_delete",
}
...
async def create_interrupt(req: Request):
    ...
    con = get_conn()
    try:
        con.execute("""INSERT INTO hitl_queue(id,agent_id,action_type,action_summary,action_data,risk_level,confidence,undo_state)
        VALUES (?,?,?,?,?,?,?,?)""",
        (interrupt_id, agent_id, action_type, action_summary,
        json.dumps(action_data,default=str)[:4000], risk_level, confidence,
        undo_state[:4000]))
        con.commit()
    finally:
        con.close()
...
async def decide_interrupt(interrupt_id: str, req: Request):
    ...
    try:
        ...
        con.execute("""UPDATE hitl_queue SET status=?,reviewer=?,review_note=?,reviewed_at=CURRENT_TIMESTAMP WHERE id=? AND status='pending'""",
                    (decision, reviewer, note, interrupt_id))
        # FIX 1: only write audit if UPDATE actually changed a row
        if con.execute("SELECT changes()").fetchone()[0] > 0:
            con.execute("INSERT INTO hitl_audit(interrupt_id,decision,reviewer,note) VALUES (?,?,?,?)",
...
async def save_undo_snapshot(req: Request):
    ...
    try:
        ...
        con.execute("INSERT INTO undo_snapshots(id,action_id,state_type,state_data) VALUES (?,?,?,?)",
...
def get_all_queue(limit: int = 100):
    ...
    try:
        rows = con.execute(
            "SELECT * FROM hitl_queue ORDER BY created_at DESC LIMIT ?",
            (min(limit,500),)
        ).fetchall()
    finally:
        con.close()
...
Return ONLY valid JSON."""
    ...
    result = await llm_svc.complete([{"role":"user","content":prompt}], agent_id="hitl", max_tokens=300, temperature=0.1, inject_steering=False)
    ...
    if m:
        try:
            ...
            pass
    return {"ok":True,"confidence":0.5,"risk_level":"medium","is_reversible":True,"concerns":[],"recommendation":"proceed"}
