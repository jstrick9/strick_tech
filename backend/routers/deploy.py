...
import asyncio, io, json, os, zipfile, logging, time
...
async def deploy_netlify(req: Request):
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            ...
            return {"ok": False, "provider": "netlify",
                    "error": data.get("message", str(data))[:200]}
    except Exception as e:
...
async def start_tunnel(req: Request):
    ...
    """
    import shutil, subprocess, asyncio, re
    ...
    try:
        ...
        if url:
            ...
            from ..services.memory_db import memory_add, audit_log
...
def deploy_history(limit: int = 20):
    ...
    from ..services.memory_db import memory_search_fts, get_conn
...
async def deploy_railway(req: Request):
    ...
    try:
        ...
        if proc.returncode == 0:
            from ..services.memory_db import memory_add
            memory_add("deploy:railway", f"Deployed via Railway CLI", "deploy,railway")
            return {
                "ok":       True,
                "provider": "railway",
                "output":   output[:500],
...
_BINARY_EXTS = {'.png','.jpg','.jpeg','.gif','.ico','.svg','.woff','.woff2','.ttf',
