...
def list_integrations(category: str = "", q: str = ""):
    ...
    if q:
        ...
        results = [i for i in results if
                   ...
                   ql in i["description"].lower() or
                   any(ql in t for t in i["tags"])]
    return results
...
async def scaffold_integration(integration_id: str, req: Request):
    ...
    messages = [
        {"role": "system", "content":
            f"You are an expert developer scaffolding {integration['name']} for a {framework} project. "
        ...
        {"role": "user", "content":
            f"Scaffold {integration['name']} for a {framework} project.\n"
...
Include: overview diagram (ASCII), folder structure explanation, key design decisions, data flow.""",
    ...
    messages = [
        {"role": "system", "content": "You are a technical writer generating professional documentation. Return only the markdown content, no preamble."},
        {"role": "user",   "content": prompt}
    ]

    result      = await llm.complete(messages, agent_id="researcher", max_tokens=3000,
                                     temperature=0.4, inject_steering=False)
...
async def stripe_wire(req: Request):
    ...
    msgs = [
        {"role": "system", "content":
            "You are a senior full-stack developer. Generate complete, production-ready Stripe integration code. "
            "Include error handling, loading states, and best practices. Use Stripe.js v3."},
        {"role": "user", "content":
            f"Generate a complete Stripe {mode} integration for '{product_name}' "
    ...
    result = await llm_svc.complete(msgs, agent_id="builder", max_tokens=3000,
                                    temperature=0.2, inject_steering=False)
...
async def stripe_products():
    ...
    return {"data": [
        {"id": "price_starter_monthly", "nickname": "Starter Monthly", "unit_amount": 999,
         "currency": "usd", "recurring": {"interval": "month"},
         "product": {"name": "Starter Plan", "description": "Up to 3 projects"}},
        {"id": "price_pro_monthly", "nickname": "Pro Monthly", "unit_amount": 2999,
         "currency": "usd", "recurring": {"interval": "month"},
         "product": {"name": "Pro Plan", "description": "Unlimited projects + API"}},
        {"id": "price_enterprise", "nickname": "Enterprise", "unit_amount": 9900,
         "currency": "usd", "recurring": {"interval": "month"},
...
async def auth_wire(req: Request):
    ...
    from ..services import llm as llm_svc
    msgs = [
        {"role": "system", "content": "You are a senior full-stack developer. Generate complete, secure auth integration code."},
        {"role": "user", "content":
            f"Generate a complete {provider} authentication integration.\n\n"
...
def auth_providers():
    ...
    providers = [
        {"id": "nextauth",  "name": "NextAuth.js",             "icon": "🔐", "difficulty": "medium",
         "env_vars": ["NEXTAUTH_SECRET", "NEXTAUTH_URL", "GOOGLE_CLIENT_ID", "GITHUB_CLIENT_ID"],
         "docs": "https://next-auth.js.org"},
        {"id": "clerk",     "name": "Clerk",                   "icon": "👤", "difficulty": "easy",
         "env_vars": ["NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY", "CLERK_SECRET_KEY"],
         "docs": "https://clerk.com/docs"},
        {"id": "supabase",  "name": "Supabase Auth",           "icon": "🚀", "difficulty": "easy",
         "env_vars": ["SUPABASE_URL", "SUPABASE_ANON_KEY"],
         "docs": "https://supabase.com/docs/guides/auth"},
        {"id": "firebase",  "name": "Firebase Auth",           "icon": "🔥", "difficulty": "medium",
         "env_vars": ["FIREBASE_API_KEY", "FIREBASE_AUTH_DOMAIN", "FIREBASE_PROJECT_ID"],
         "docs": "https://firebase.google.com/docs/auth"},
        {"id": "auth0",     "name": "Auth0",                   "icon": "🛡️", "difficulty": "easy",
         "env_vars": ["AUTH0_DOMAIN", "AUTH0_CLIENT_ID", "AUTH0_CLIENT_SECRET"],
         "docs": "https://auth0.com/docs"},
        {"id": "magic",     "name": "Magic Link (Passwordless)","icon": "✉️", "difficulty": "easy",
