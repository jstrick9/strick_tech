...
def _guess_platform(ext: str) -> str:
    return {".dmg":"macOS",".app":"macOS",".exe":"Windows",
            ".msi":"Windows",".deb":"Linux",".AppImage":"Linux",".rpm":"Linux"}.get(ext,"Unknown")
...
def tauri_status():
    ...
    return {
        ...
        "python":       {"available": py_ok, "version": f"{os.sys.version_info.major}.{os.sys.version_info.minor}.{os.sys.version_info.micro}"},
...
async def start_build(target: str = "all"):
    ...
    if not _rust_available():
        return {"ok": False, "error": "Rust not installed. Run: curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh"}
    ...
    if not conf.exists():
        return {"ok": False, "error": "tauri.conf.json not found. Run the Tauri setup first.",
...
async def install_tauri_cli():
    ...
    if not _rust_available():
        ...
        return StreamingResponse(_no_rust(), media_type="text/event-stream",
