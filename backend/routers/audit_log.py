...
def _issue_receipt(con, entry_id: str, agent_id: str, entry_hash: str) -> str:
    ...
    try:
        row = con.execute(
            'SELECT public_key, signing_key FROM agent_identities WHERE agent_id=?', (agent_id,)
        ).fetchone()
        if row and row['signing_key']:
            # HMAC-SHA256 signature using the stored signing key
            sig = hmac.new(row['signing_key'].encode('utf-8'), entry_hash.encode('utf-8'), hashlib.sha256).hexdigest()
            pub_key = row['public_key'] or ''
    except (sqlite3.Error, KeyError, TypeError, AttributeError):
...
def list_entries(
    ...
    try:
        rows = con.execute(
            f'SELECT * FROM audit_log_chain {where_sql} ORDER BY seq DESC LIMIT ? OFFSET ?', params
        ).fetchall()
        total = con.execute(f'SELECT COUNT(*) FROM audit_log_chain {where_sql}', params[:-2]).fetchone()[0]
    finally:
        con.close()
...
def get_entry(entry_id: str):
    ...
    con = _get_conn()
    try:
        entry = con.execute('SELECT * FROM audit_log_chain WHERE entry_id=?', (entry_id,)).fetchone()
        receipt = con.execute('SELECT * FROM audit_receipts WHERE entry_id=?', (entry_id,)).fetchone()
    finally:
        con.close()
...
def audit_stats():
    ...
    try:
        ...
        by_risk = con.execute('SELECT risk_level, COUNT(*) as cnt FROM audit_log_chain GROUP BY risk_level').fetchall()
        by_outcome = con.execute('SELECT outcome, COUNT(*) as cnt FROM audit_log_chain GROUP BY outcome').fetchall()
        ...
        recent_hash = con.execute('SELECT entry_hash FROM audit_log_chain ORDER BY seq DESC LIMIT 1').fetchone()
    finally:
        con.close()
