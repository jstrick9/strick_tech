...
@router.get('')
def list_webhooks():
    """Retrieve and return list webhooks."""
    con = get_conn()
    try:
...
def delete_webhook(webhook_id: str):
    """Delete or remove specified webhook."""
    con = get_conn()
    try:
...
def webhook_events(webhook_id: str, limit: int = 20):
    """Execute or process webhook events operation."""
    con = get_conn()
    try:
        rows = con.execute(
        ...
        log.error('webhook_events DB error: %s', e)
        return []
    finally:
        con.close()
...
async def trigger_webhook(
    ...
    if 'repository' in payload and 'pusher' in payload:
        ...
        'type' in payload
        and payload.get('type', '').startswith('customer')
...
async def _run_webhook_agent(wh: dict, prompt: str, run_id: str, webhook_id: str, source: str, payload: str):
    ...
    system = (
        agent.get('system_prompt') or f'You are {agent.get("name", agent_id)}, processing an automated webhook event.'
    )
...
def webhook_templates():
    """Execute or process webhook templates operation."""
    return [
        {
            'id': 'github-push',
