...
from ..services import memory_db, llm
...
async def update_agent(agent_id: str, req: Request):
    ...
    allowed = {"name", "role", "model", "provider", "color", "avatar",
               "system_prompt", "status", "enabled"}
    data = {"id": agent_id}
    for k in allowed:
        if k in body:
            v = body[k]
            if k == "name":          v = str(v).strip()[:80]
            elif k == "role":        v = str(v)[:200]
            elif k == "model":       v = str(v)[:100]
            elif k == "provider":    v = str(v)[:32]
            elif k == "color":       v = str(v)[:16]
            elif k == "avatar":      v = str(v)[:8]
            elif k == "system_prompt": v = str(v)[:4000]
            elif k == "enabled":     v = int(bool(v))
            data[k] = v
...
async def test_agent(agent_id: str, req: Request):
    ...
    result = await llm.complete(messages, agent_id=agent.get("model") or agent_id,
                                 model=agent.get("model", ""), max_tokens=512, inject_steering=False)
    return {
        "ok": result.get("ok"),
...
def _random_color() -> str:
    ...
    colors = ["#7aa2f7","#bb9af7","#9ece6a","#f7768e","#e0af68",
              "#2ac3de","#ff9e64","#c084fc","#7dd3a7","#f5c542"]
