...
import threading as _hmr_threading_mod, asyncio as _asyncio
...
def git_status():
    ...
    try:
        ...
        return {
            ...
            "unstaged":  unstaged,
            "recent_commits": [
                {"hash": c.hexsha[:8], "message": c.message.strip()[:80],
                 "author": str(c.author), "date": c.committed_datetime.isoformat()[:10]}
                for c in commits
            ],
        }
    except ImportError:
        ...
        for proc in psutil.process_iter(["pid", "name", "status", "memory_info"]):
            ...
            if any(k in name.lower() for k in ["python", "uvicorn", "node", "ollama"]):
                procs.append({
                    ...
                    "ram_mb": round((proc.info.get("memory_info") or type("",(),{"rss":0})()).rss / 1024 / 1024, 1),
                })
        return procs[:10]
    except Exception:
        return []
