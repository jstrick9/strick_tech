...
def _transform(op_a: list, op_b: list, side: str = "left") -> tuple[list, list]:
    ...
    while i < len(ai) or j < len(bi):
        ...
        if isinstance(pa, int) and pa > 0 and isinstance(pb, int) and pb > 0:
            n = min(pa, pb)
            a_prime.append(n); b_prime.append(n)
            ai[i] = pa - n; bi[j] = pb - n
            if ai[i] == 0: i += 1
            if bi[j] == 0: j += 1
            continue
...
def _compose(op_a: list, op_b: list) -> list:
    ...
    while i < len(ai) or j < len(bi):
        ...
        if isinstance(pb, str):                        # B insert
            result.append(pb); j += 1; continue
        if isinstance(pa, int) and pa < 0:             # A delete
            result.append(pa); i += 1; continue
        if pa is None: result.append(pb); j += 1; continue
        if pb is None: result.append(pa); i += 1; continue
...
def _compact(op: list) -> list:
    ...
    result: list = []
    for c in op:
        if not c and not isinstance(c, str): continue  # skip zero-retains
        if result and isinstance(result[-1], type(c)) and not isinstance(c, str):
            result[-1] += c
        elif result and isinstance(result[-1], str) and isinstance(c, str):
            result[-1] += c
        else:
            result.append(c)
...
class CRDTDoc:
    ...
    async def apply_and_broadcast(self, peer_id: str, peer_name: str,
                                   client_rev: int, op: list) -> dict:
        """
    ...
    async def undo(self, peer_id: str) -> Optional[dict]:
        ...
        if not stack:
            return None
        entry = stack.pop()
        return await self.apply_and_broadcast(peer_id, f"{peer_id}(undo)",
                                              self.revision, entry["op"])
    ...
    async def redo(self, peer_id: str) -> Optional[dict]:
        ...
        if not stack:
            return None
        entry = stack.pop()
        return await self.apply_and_broadcast(peer_id, f"{peer_id}(redo)",
                                              self.revision, entry["op"])
        ...
        try:
            ...
            try:
                con.execute(
                    ...
                    (self.id, entry["revision"], entry["peer_id"],
                     entry["peer_name"], json.dumps(entry["op"]))
                )
                con.commit()
            finally:
                con.close()
...
def _get_doc(doc_id: str) -> CRDTDoc:
    ...
    try:
        ...
        ops = con.execute(
            "SELECT revision,peer_id,peer_name,op_json FROM crdt_ops WHERE doc_id=? ORDER BY revision",
            (doc_id,)
        ).fetchall()
    finally:
        con.close()
    if row:
        ...
        doc.ops_log  = [{"revision":r["revision"],"peer_id":r["peer_id"],
                         "peer_name":r["peer_name"],"op":json.loads(r["op_json"])}
                        for r in ops]
    else:
        doc = CRDTDoc(doc_id)
    _docs[doc_id] = doc
...
def get_history(doc_id: str, limit: int = 100):
    ...
    return {
        "history": [
            {**dict(r), "op": json.loads(r["op_json"])}
            for r in rows
        ],
        "count": len(rows),
    }
...
async def restore_revision(doc_id: str, revision: int):
    ...
    try:
        ops = con.execute(
            "SELECT op_json FROM crdt_ops WHERE doc_id=? AND revision<=? ORDER BY revision",
            (doc_id, revision)
        ).fetchall()
    finally:
        con.close()
