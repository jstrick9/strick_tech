...
async def create_goal(req: Request):
    ...
    try:
        ...
        append_entry("user", "User", "goal_created", f"Goal: {title}",
...
async def update_goal(goal_id: str, req: Request):
    ...
    allowed = {"title","description","success_criteria","domain","priority",
               "status","progress","deadline","tags","notes","supervisor_run_id","spec_id"}
    updates = {}
    for k, v in body.items():
        if k in allowed:
            if k == "progress":
                ...
                continue
            elif k == "domain" and v not in DOMAINS:
                continue
            else:
    ...
    try:
        ...
        con.execute(f"UPDATE goals_v2 SET {sets} WHERE id=?",
    ...
    try:
        ...
        asyncio.create_task(broadcast({
            "type": "goal_updated", "goal_id": goal_id,
            "progress": updates.get("progress"), "status": updates.get("status"),
        }))
    except Exception:
        pass
...
async def launch_goal(goal_id: str, req: Request):
    ...
    if run_data.get("ok"):
        ...
        con = _get_conn()
        try:
            con.execute("UPDATE goals_v2 SET supervisor_run_id=?, status='active', updated_at=? WHERE id=?",
                        (run_id, _now(), goal_id))
            con.commit()
        finally:
            con.close()
...
async def add_checkin(goal_id: str, req: Request):
    ...
    try:
        ...
        if progress > 0:
            con.execute("UPDATE goals_v2 SET progress=?, updated_at=? WHERE id=?",
...
async def add_milestone(goal_id: str, req: Request):
    ...
    try:
        con.execute("""
            INSERT INTO goal_milestones (id,goal_id,title,description,due_date,created_at)
            VALUES (?,?,?,?,?,?)
        """, (ms_id, goal_id, title[:200],
              (body.get("description") or "")[:500],
              (body.get("due_date") or "")[:30], _now()))
        con.commit()
    finally:
        con.close()
...
def complete_milestone(goal_id: str, ms_id: str):
    ...
    try:
        con.execute(
            "UPDATE goal_milestones SET completed=1, completed_at=? WHERE id=? AND goal_id=?",
            (_now(), ms_id, goal_id)
        )
        # Recompute progress from milestones
        total = con.execute("SELECT COUNT(*) FROM goal_milestones WHERE goal_id=?", (goal_id,)).fetchone()[0]
        done  = con.execute("SELECT COUNT(*) FROM goal_milestones WHERE goal_id=? AND completed=1", (goal_id,)).fetchone()[0]
        if total > 0:
            progress = int((done / total) * 100)
            status = "done" if progress >= 100 else "active"
            con.execute("UPDATE goals_v2 SET progress=?, status=?, updated_at=? WHERE id=?",
                        (progress, status, _now(), goal_id))
        con.commit()
    finally:
        con.close()
...
def goals_summary():
    ...
    try:
        ...
        by_status= con.execute(
            "SELECT status, COUNT(*) as cnt FROM goals_v2 GROUP BY status"
        ).fetchall()
        by_domain= con.execute(
            "SELECT domain, COUNT(*) as cnt FROM goals_v2 GROUP BY domain ORDER BY cnt DESC"
        ).fetchall()
        by_prio  = con.execute(
            "SELECT priority, COUNT(*) as cnt FROM goals_v2 GROUP BY priority"
        ).fetchall()
...
async def decompose_goal(goal_id: str, req: Request):
    ...
    if not force:
        ...
        try:
            existing = con.execute(
                "SELECT * FROM goal_decompositions WHERE goal_id=? ORDER BY seq",
                (goal_id,)
            ).fetchall()
        finally:
            con.close()
        if existing:
            ...
            return {"ok": True, "goal_id": goal_id, "tasks": tasks, "edges": edges,
                    "cached": True, "task_count": len(tasks)}
    ...
    try:
        ...
        result = await complete(
            ...
            agent_id="brain", max_tokens=1500, temperature=0.3, inject_steering=False
        )
    ...
    if not raw_tasks:
        raw_tasks = [
            {"seq":1,"title":"Research & Plan","description":f"Research best approaches for: {g['title']}","agent_hint":"researcher","depends_on":[],"risk_level":"low","est_tokens":500},
            {"seq":2,"title":"Design Solution","description":f"Design a concrete solution for: {g['title']}","agent_hint":"brain","depends_on":[1],"risk_level":"low","est_tokens":600},
            {"seq":3,"title":"Execute","description":f"Implement the designed solution","agent_hint":"builder","depends_on":[2],"risk_level":"low","est_tokens":800},
            {"seq":4,"title":"Review & Finalize","description":f"Review, verify, and finalize the result","agent_hint":"reviewer","depends_on":[3],"risk_level":"low","est_tokens":400},
        ]
    ...
    return {
        "ok": True, "goal_id": goal_id,
        "tasks": tasks_out, "edges": edges,
        "cached": False, "task_count": len(tasks_out),
    }
        ...
        for dep_seq in (t.get("depends_on") or []):
            ...
            if src:
                edges.append({
                    ...
                    "from_id": src["id"], "to_id": t["id"],
                    "from_seq": dep_seq, "to_seq": t["seq"],
                })
    return edges
...
def get_decomposition(goal_id: str):
    ...
    edges = _build_decomp_edges(tasks_out)
    return {"ok": True, "goal_id": goal_id, "tasks": tasks_out,
            "edges": edges, "task_count": len(tasks_out)}
...
async def score_goal_outcome(goal_id: str, req: Request):
    ...
    try:
        ...
        result = await complete(
            ...
            agent_id="reviewer", max_tokens=500, temperature=0.1, inject_steering=False
        )
        text = result.get("text", "")
        m = re.search(r'\{.*\}', text, re.DOTALL)
        if m:
    ...
    try:
        con.execute("""
            ...
            VALUES (?,?,?,?,?,?,?,?)
        """, (goal_id, iteration, overall,
              json.dumps(dims), summary, "evaluator",
              run_id or "", now))
    ...
    try:
        con.execute(
            "INSERT INTO goal_checkins (goal_id,agent_id,note,progress,created_at) VALUES (?,?,?,?,?)",
            (goal_id, "evaluator",
             f"Outcome score: {int(overall*100)}% ({grade}) — {summary[:200]}", rec_progress, now)
        )
        con.commit()
    finally:
        con.close()
...
def get_score_history(goal_id: str):
    ...
    try:
        ...
        if not row:
            return JSONResponse({"ok": False, "error": "Goal not found"}, status_code=404)
        history = con.execute(
            "SELECT * FROM goal_score_history WHERE goal_id=? ORDER BY iteration",
            (goal_id,)
        ).fetchall()
    finally:
        con.close()
...
def get_latest_score(goal_id: str):
    ...
    try:
        ...
        if not row:
            return JSONResponse({"ok": False, "error": "Goal not found"}, status_code=404)
        latest = con.execute(
            "SELECT * FROM goal_score_history WHERE goal_id=? ORDER BY iteration DESC LIMIT 1",
            (goal_id,)
        ).fetchone()
    finally:
        con.close()
    ...
    if not latest:
        return {
            "ok": True, "goal_id": goal_id, "scored": False,
...
def get_goal_full(goal_id: str):
    ...
    for field in ("score_breakdown", "decomposition"):
        try:
            g[field] = json.loads(g.get(field) or "{}") if field == "score_breakdown" else json.loads(g.get(field) or "[]")
        except Exception:
