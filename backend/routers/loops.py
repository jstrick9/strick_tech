...
def scheduler_status():
    sched = sched_svc.get_scheduler()
    user_jobs = [j for j in (sched.get_jobs() if sched else [])
                 if j.id not in _BUILTIN_JOB_IDS]
    builtin_jobs = [j for j in (sched.get_jobs() if sched else [])
                    if j.id in _BUILTIN_JOB_IDS]
    return {
