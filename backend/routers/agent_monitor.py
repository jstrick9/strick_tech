...
import asyncio, json, logging, statistics, time, uuid
...
def update_agent_status(agent_id: str, **kwargs):
    ...
    try:
        existing = con.execute(
            "SELECT agent_id FROM agent_live_status WHERE agent_id=?", (agent_id,)
        ).fetchone()
        ...
        if not existing:
            ...
            con.execute(f"INSERT INTO agent_live_status ({cols}) VALUES ({vals})",
                        list(kwargs.values()))
        else:
...
def record_kpi_snapshot(agent_id: str, period: str = "hour"):
    ...
    try:
        ...
        con.execute("""
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
        """, (agent_id, period, now, total, success, failed,
              perf["avg_lat"] or 0, perf["tok"] or 0,
              perf["cost"] or 0, eval_avg or 0, err_rate, now))
        con.commit()
    finally:
        con.close()
    ...
    try:
        ...
        if cur["total"] > 0 and base["avg_lat"]:
            ...
            if err_rate > ANOMALY_THRESHOLDS["error_rate"]["crit"]:
                anomalies.append({"type": "error_rate", "severity": "critical",
                                   "val": err_rate, "baseline": base["err_rate"] or 0})
            elif err_rate > ANOMALY_THRESHOLDS["error_rate"]["warn"]:
                anomalies.append({"type": "error_rate", "severity": "warning",
                                   "val": err_rate, "baseline": base["err_rate"] or 0})
            ...
            if base["avg_lat"] and cur["avg_lat"]:
                ...
                if ratio > ANOMALY_THRESHOLDS["latency_spike"]["crit"]:
                    anomalies.append({"type": "latency_spike", "severity": "critical",
                                       "val": cur["avg_lat"], "baseline": base["avg_lat"]})
                elif ratio > ANOMALY_THRESHOLDS["latency_spike"]["warn"]:
                    anomalies.append({"type": "latency_spike", "severity": "warning",
                                       "val": cur["avg_lat"], "baseline": base["avg_lat"]})
            ...
            if base["avg_cost"] and cur["avg_cost"] and base["avg_cost"] > 0:
                ...
                if ratio > ANOMALY_THRESHOLDS["cost_spike"]["warn"]:
                    ...
                    anomalies.append({"type": "cost_spike", "severity": sev,
                                       "val": cur["avg_cost"], "baseline": base["avg_cost"]})
        ...
        if live and live["heartbeat_at"]:
            try:
                ...
                if silence_s > ANOMALY_THRESHOLDS["silence"]["crit"]:
                    anomalies.append({"type": "silence", "severity": "critical",
                                       "val": silence_s, "baseline": 60})
                elif silence_s > ANOMALY_THRESHOLDS["silence"]["warn"]:
                    anomalies.append({"type": "silence", "severity": "warning",
                                       "val": silence_s, "baseline": 60})
            except Exception:
                pass
        ...
        for a in anomalies:
            con.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?)
            """, (agent_id, a["type"], a["severity"],
                  f"{a['type']}: {a['val']:.3f} vs baseline {a['baseline']:.3f}",
                  a["type"], a["val"], a["baseline"], now))
        ...
        con.execute("UPDATE agent_live_status SET anomaly_score=?, anomaly_flags=? WHERE agent_id=?",
                    (score, flags, agent_id))
        con.commit()
    finally:
        con.close()
...
def live_dashboard():
    ...
    try:
        ...
        live   = {r["agent_id"]: dict(r) for r in
                  con.execute("SELECT * FROM agent_live_status").fetchall()}
        killed = {r["agent_id"] for r in
                  con.execute("SELECT agent_id FROM agent_kill_switches").fetchall()}
        ...
        for a in agents:
            ...
            stat = live.get(aid, {})
            flags = []
            try: flags = json.loads(stat.get("anomaly_flags","[]"))
            except: pass
            result.append({
                "agent_id":     aid,
                "name":         a["name"],
...
def agent_kpis(agent_id: str, period: str = "hour", limit: int = 24):
    ...
    try:
        ...
        """, (agent_id, period, min(limit, 200))).fetchall()
...
def list_anomalies(agent_id: str = "", severity: str = "", resolved: bool = False, limit: int = 50):
    ...
    try:
        rows = con.execute(
            f"SELECT * FROM anomaly_events WHERE {' AND '.join(where)} ORDER BY created_at DESC LIMIT ?",
            params
        ).fetchall()
    finally:
        con.close()
...
def resolve_anomaly(anomaly_id: int):
    con = _get_conn()
    try:
        con.execute("UPDATE anomaly_events SET resolved=1, resolved_at=? WHERE id=?",
                    (_now(), anomaly_id))
        con.commit()
    finally:
        con.close()
...
async def kill_agent(agent_id: str, req: Request):
    ...
    con = _get_conn()
    try:
        con.execute("""INSERT OR REPLACE INTO agent_kill_switches (agent_id,killed_at,reason,killed_by)
                       VALUES (?,?,?,?)""", (agent_id, _now(), reason, killed_by))
    ...
    append_entry("user", "User", "agent_killed",
                 f"Agent {agent_id} killed: {reason}",
                 authority="user", risk_level="high", outcome="blocked",
...
async def create_shadow_test(req: Request):
    ...
    con = _get_conn()
    try:
        con.execute("""INSERT INTO shadow_tests (test_id,agent_id,shadow_config,status,created_at)
                       VALUES (?,?,?,?,?)""",
                    (test_id, agent_id, json.dumps(config), "running", _now()))
        con.commit()
    finally:
        con.close()
    return {"ok": True, "test_id": test_id, "agent_id": agent_id,
