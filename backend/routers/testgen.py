...
{f'Additional context: {context}' if context else ''}"""
    ...
    messages = [
        ...
        {"role": "user",   "content": f"Write {framework} tests for this {ext} code:\n\n```{ext}\n{source_code[:6000]}\n```"},
    ]
    ...
    if stream:
        ...
        async def generate():
            async for chunk in llm.stream(messages, agent_id="reviewer", max_tokens=4096, temperature=0.2, inject_steering=False):  # FIX 2a
                yield chunk
    ...
    # Non-streaming
    result   = await llm.complete(messages, agent_id="reviewer", max_tokens=4096, temperature=0.2, inject_steering=False)  # FIX 2b
...
async def generate_project_tests(req: Request):
    ...
    files = [
        ...
        and "test" not in f.name.lower() and "spec" not in f.name.lower()
...
def list_frameworks():
    return [
        {"id": k, "lang": v["lang"], "ext": v["ext"],
         "description": f"{k.capitalize()} test framework for {v['lang']}"}
...
def generate_project_tests_get(framework: str = "pytest", max_files: int = 10):
    ...
    return {"ok": True, "framework": framework, "test_files": test_files, "count": len(test_files),
