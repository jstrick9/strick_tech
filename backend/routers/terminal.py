...
SAFE_PREFIXES = {
    "ls", "cat", "echo", "pwd", "git", "node", "npm", "npx", "pip",
    "pip3", "python", "python3", "mkdir", "touch", "mv", "cp",
    "find", "grep", "head", "tail", "wc", "curl", "wget",
    "which", "env", "export", "cd", "clear",
}
...
def _store_history(session_id: str, command: str, cwd: str):
    try:
        con = get_conn()
        try:
            con.execute("INSERT INTO terminal_history(session_id,command,cwd) VALUES(?,?,?)",
            (session_id, command[:500], cwd[:200]))
            # Keep only last 500 per session
            con.execute("DELETE FROM terminal_history WHERE id NOT IN (SELECT id FROM terminal_history WHERE session_id=? ORDER BY id DESC LIMIT 500)", (session_id,))
            con.commit()
        finally:
            con.close()
    except Exception:
