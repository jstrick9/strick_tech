...
import asyncio, datetime, json, base64, os, socket, io
...
async def preview_save(req: Request):
    ...
    if old != content:
        con.execute(
            ...
            (path, old, d.get("author","builder"), d.get("message","save")[:240])  # FIX 2: store pre-save state (even if ""), not new content
        )
...
async def preview_restore(req: Request):
    ...
    con = DB()
    row = con.execute(
        "SELECT path,content FROM file_versions WHERE id=?", (d.get("version_id"),)
    ).fetchone()
    con.close()
    if not row:
        return {"ok": False}
...
async def agent_fix(req: Request):
    ...
    messages = [
        {"role": "system", "content":
            f"You are debugging a {language} file. Fix the error and return ONLY the corrected code."},
...
async def complete_code(req: Request):
    ...
    if not language:
        ...
        language = {"tsx":"typescript","jsx":"javascript","ts":"typescript",
                    "py":"python","css":"css","md":"markdown"}.get(ext, ext)

    messages = [
        {"role": "system", "content":
            f"Complete the {language} code. Return ONLY the completion text, no explanation."},
        {"role": "user", "content": f"FILE: {filepath}\n<PREFIX>{prefix}</PREFIX>\n<SUFFIX>{suffix}</SUFFIX>\nCOMPLETION:"}
    ]
    result = await llm.complete(messages, agent_id="free", max_tokens=200, temperature=0.3, timeout=8.0, inject_steering=False)  # FIX L
...
<main class="max-w-4xl mx-auto px-6 py-16">
  ...
  <div class="grid grid-cols-3 gap-4" id="features">
    {''.join(f'<div class="bg-gray-900 border border-gray-800 rounded-xl p-5"><div class="text-2xl mb-2">{e}</div><div class="font-semibold mb-1">{t}</div><div class="text-gray-400 text-sm">{d}</div></div>' for e,t,d in [
...
<script src="app.js" type="module"></script>
        ...
        write_rel("app.js", f"""// {title} — Agentic OS
...
""")
        ...
        write_rel("package.json", json.dumps({
            "name": "agentic-next-app", "version": "0.1.0", "private": True,
            "scripts": {"dev":"next dev","build":"next build","start":"next start"},
            "dependencies": {"next":"15.0.0","react":"^18","react-dom":"^18",
                             "tailwindcss":"^3.4.1","clsx":"^2.1.0","lucide-react":"^0.400.0"},
            "devDependencies": {"typescript":"^5","@types/react":"^18","@types/node":"^20","postcss":"^8","autoprefixer":"^10"}
        }, indent=2))
        write_rel("app/page.tsx", f"""export default function Home() {{
  return (
...
""")
        ...
        write_rel("mobile/App.jsx", f"""import React, {{ useState }} from 'react';
...
""")
        write_rel("mobile/package.json", json.dumps({
            "name": "agentic-expo-app", "version": "1.0.0", "main": "expo-router/entry",
        ...
        write_rel("package.json", json.dumps({
            "name": "agentic-sveltekit", "version": "0.0.1", "type": "module",
            "scripts": {"dev":"vite dev","build":"vite build","preview":"vite preview"},
            "devDependencies": {"@sveltejs/adapter-auto":"^3.0.0","@sveltejs/kit":"^2.0.0",
                                "svelte":"^4.2.7","vite":"^5.0.3","tailwindcss":"^3.4.1"}
        }, indent=2))
        write_rel("src/routes/+page.svelte", f"""<script lang="ts">
  let count = 0;
</script>
    ...
    for path in [PREVIEW_DIR / "package.json", PREVIEW_DIR / "app" / "package.json"]:
        if path.exists():
            try:
                ...
                return {"ok": True, "path": str(path), "name": data.get("name",""),
                        "version": data.get("version",""), "dependencies": deps, "devDependencies": dev_deps}
            except Exception:
                pass
...
async def pm_add(req: Request):
    ...
    # create package.json
    pkg = {"name": "agentic-app", "version": "0.1.0",
...
async def pm_search(q: str = "", size: int = 12):
    ...
    CURATED = [
        {"name":"react","version":"18.3.1","description":"A JavaScript library for building user interfaces","score":0.99},
        ...
        {"name":"clsx","version":"2.1.0","description":"Utility for constructing className strings","score":0.83},
        {"name":"date-fns","version":"3.6.0","description":"Modern JavaScript date utility library","score":0.82},
    ]
    if not q:
        return {"results": CURATED[:size]}
    filtered = [p for p in CURATED if q.lower() in p["name"].lower() or q.lower() in p["description"].lower()]
    return {"results": filtered[:size] or [{"name": q, "version": "latest", "description": "Package from npm registry", "score": 0.5}]}
