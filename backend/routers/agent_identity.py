...
def provision_agent_identity(agent_id: str, display_name: str = '', authority_level: str = 'standard') -> dict:
    ...
    con = _get_conn()
    try:
        existing = con.execute('SELECT agent_id FROM agent_identities WHERE agent_id=?', (agent_id,)).fetchone()
...
def get_agent_identity(agent_id: str) -> dict | None:
    ...
    try:
        ...
        if not row:
            return None
        perms = con.execute('SELECT action, resource FROM agent_permissions WHERE agent_id=?', (agent_id,)).fetchall()
...
def issue_jit_token(
    ...
    try:
        identity = con.execute(
            'SELECT agent_id, status, signing_key FROM agent_identities WHERE agent_id=?', (agent_id,)
        ).fetchone()
        if not identity:
            con.close()
        ...
        # Update last_seen
        con.execute('UPDATE agent_identities SET last_seen_at=? WHERE agent_id=?', (_now_iso(), agent_id))
        _log_identity_event(con, agent_id, 'jit_token_issued', f'task={task_id} ttl={ttl_seconds}s scope={scope_str}')
        con.commit()
    finally:
        con.close()
...
def provision_all_agents() -> list[dict]:
    ...
    for agent in agents:
        ...
        if not existing:
            result = provision_agent_identity(agent['id'], agent['name'], authority)
...
async def rotate_keys(agent_id: str, req: Request):
    ...
    con = _get_conn()
    try:
        existing = con.execute('SELECT key_version FROM agent_identities WHERE agent_id=?', (agent_id,)).fetchone()
        if not existing:
            con.close()
            return JSONResponse({'ok': False, 'error': 'Identity not found'}, status_code=404)
        ...
        con.execute(
            'UPDATE agent_jit_tokens SET revoked=1, revoked_at=? WHERE agent_id=? AND revoked=0', (now, agent_id)
        )
        revoked_count = con.execute('SELECT changes()').fetchone()[0]
        _log_identity_event(con, agent_id, 'keys_rotated', f'new_version={new_version} revoked_tokens={revoked_count}')
        con.commit()
    finally:
        con.close()
...
def list_permissions(agent_id: str):
    ...
    con = _get_conn()
    try:
        perms = con.execute('SELECT * FROM agent_permissions WHERE agent_id=? ORDER BY action', (agent_id,)).fetchall()
    finally:
        con.close()
