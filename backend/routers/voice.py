...
VOICE_COMMANDS = [
    ...
    {"pattern": r"(?:create|new|make)\s+(?:a\s+)?(?:file\s+)?(?:called\s+)?(.+\.(?:py|js|ts|html|css|json|md|yaml|yml|sh|txt))",
     "action": "create_file", "example": "create a file called app.py"},
    {"pattern": r"(?:open|edit|show)\s+(.+\.(?:py|js|ts|html|css|json|md|yaml|yml|sh|txt))",
     "action": "open_file", "example": "open index.html"},

    # Agent control
    {"pattern": r"(?:run|start|execute)\s+(?:the\s+)?(.+?)\s+agent",
     "action": "run_agent", "example": "run the researcher agent"},
    {"pattern": r"(?:stop|kill|cancel)\s+(?:all\s+)?agents?",
     "action": "stop_agents", "example": "stop all agents"},

    # Workflow — before run tests to avoid conflict
    {"pattern": r"(?:run|start|execute)\s+(?:workflow|flow)\s+(.+)",
     "action": "run_workflow", "example": "run workflow code review"},

    # Code actions
    {"pattern": r"(?:run|execute)\s+(?:the\s+)?tests?",
     "action": "run_tests", "example": "run tests"},
    {"pattern": r"(?:deploy|ship|publish)\s+(?:the\s+)?(?:app|project)?",
     "action": "deploy", "example": "deploy the app"},
    {"pattern": r"(?:save|commit)\s+(?:the\s+)?(?:file|changes|everything)?",
     "action": "save", "example": "save the file"},
    {"pattern": r"(?:undo|revert)\s+(?:last\s+)?(?:change|edit)?",
     "action": "undo", "example": "undo last change"},

    # Chat — must be before navigate "tell" could match
    {"pattern": r"(?:send|say|tell the agent|tell ai|tell agentic)\s+(.+)",
     "action": "chat_send", "example": "tell the agent to build a login form"},

    # Search
    {"pattern": r"(?:search|find|look for)\s+(?:for\s+)?(.+)",
     "action": "search", "example": "search for authentication"},
    ...
    {"pattern": r"(?:change|switch|use)\s+(?:model\s+to\s+|to\s+model\s+|the\s+model\s+to\s+)?(.+?)\s+model",
     "action": "change_model", "example": "change to GPT-4 model"},
    ...
    {"pattern": r"(?:open|show)\s+(?:the\s+)?(?:command\s+)?palette",
     "action": "command_palette", "example": "open command palette"},
    {"pattern": r"(?:toggle|show|hide)\s+sidebar",
     "action": "toggle_sidebar", "example": "toggle sidebar"},
    {"pattern": r"(?:new|create)\s+(?:a\s+)?(?:chat|conversation)",
     "action": "new_chat", "example": "new chat"},
    {"pattern": r"(?:clear|reset)\s+(?:the\s+)?chat",
     "action": "clear_chat", "example": "clear chat"},

    # Navigation — multi-word pane names captured with .+? then resolved
    {"pattern": r"(?:go to|navigate to|show)\s+(?:the\s+)?(.+?)(?:\s+pane|\s+tab)?$",
     "action": "navigate", "example": "go to chat / navigate to knowledge graph"},
    {"pattern": r"(?:open|switch|change)\s+(?:to\s+)?(?:the\s+)?(.+?)(?:\s+pane|\s+tab)?$",
     "action": "navigate", "example": "open terminal / switch to kanban"},

    # Help
    {"pattern": r"(?:help|what can you do|show commands|voice commands|show help)",
     "action": "help", "example": "help / voice commands"},
]
...
async def synthesize_speech(req: Request):
    ...
    try:
        from ..routers.tts import _edge_tts, _strip_markdown, EDGE_VOICES, DEFAULT_VOICE
