...
def _update_elo(winner: str, loser: str, k: float = 32.0):
    ...
    try:
        ...
        con.execute("UPDATE arena_leaderboard SET elo=?,wins=wins+1,battles=battles+1,updated_at=CURRENT_TIMESTAMP WHERE model=?",
                    (new_elo_w, winner))
        con.execute("UPDATE arena_leaderboard SET elo=?,losses=losses+1,battles=battles+1,updated_at=CURRENT_TIMESTAMP WHERE model=?",
                    (new_elo_l, loser))
        con.commit()
    finally:
        con.close()
...
async def create_battle(req: Request):
    ...
    async def _stream():
        ...
        async def _run_model(model_id: str, model_full: str, side: str):
            ...
            if side == "a":
                resp_a = text; lat_a = lat; tok_a = toks
            else:
                resp_b = text; lat_b = lat; tok_b = toks
        ...
        c2 = _gc()
        c2.execute("""UPDATE arena_battles SET response_a=?,response_b=?,latency_a=?,latency_b=?,tokens_a=?,tokens_b=?
                      WHERE id=?""",
...
async def vote(battle_id: str, req: Request):
    ...
    try:
        ...
        con.execute("UPDATE arena_battles SET winner=?,vote_reason=? WHERE id=?",
                    (winner, reason, battle_id))
        con.commit()
    finally:
        con.close()
...
def leaderboard(limit: int = 20):
    ...
    try:
        rows = con.execute(
            "SELECT * FROM arena_leaderboard WHERE battles>0 ORDER BY elo DESC LIMIT ?",
            (min(limit,50),)
        ).fetchall()
    finally:
        con.close()
...
def arena_stats():
    ...
    try:
        ...
        top_model= con.execute("SELECT model FROM arena_leaderboard WHERE battles>0 ORDER BY elo DESC LIMIT 1").fetchone()
        most_used= con.execute("SELECT model_a as m,COUNT(*) as c FROM arena_battles GROUP BY model_a ORDER BY c DESC LIMIT 1").fetchone()
    finally:
        con.close()
    return {
        "total_battles":  total,
...
Return JSON: {{"winner": "a"|"b"|"tie", "reason": "brief explanation", "scores": {{"a": 1-10, "b": 1-10}}}}"""
    ...
    result = await llm_svc.complete(
        ...
        agent_id="judge", max_tokens=300, temperature=0.1, inject_steering=False
    )
    ...
    if m:
        try:
            ...
            from ..services.memory_db import get_conn as _gc
            c2 = _gc()
            c2.execute("UPDATE arena_battles SET winner=?,vote_reason=? WHERE id=?",
                       (winner, f"[AUTO] {reason}", battle_id))
            c2.commit(); c2.close()
            if winner in ("a","b"):
