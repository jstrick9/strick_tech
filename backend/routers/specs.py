...
async def create_spec(req: Request):
    ...
    con = get_conn()
    try:
        con.execute('INSERT INTO specs(id,title,description,workspace_id) VALUES (?,?,?,?)', (spec_id, title, desc, ws))
        con.commit()
    finally:
        con.close()
...
async def update_spec(spec_id: str, req: Request):
    ...
    con = get_conn()
    try:
        cur = con.execute(f'UPDATE specs SET {", ".join(sets)} WHERE id=?', vals)
        con.commit()
        if cur.rowcount == 0:
            return {'ok': False, 'error': 'Spec not found'}
        row = con.execute('SELECT * FROM specs WHERE id=?', (spec_id,)).fetchone()
...
- Return ONLY valid JSON array, no markdown wrapper"""
        ...
        if not tasks_json:
            tasks_json = [
                ...
                },
                {
                    ...
                    'description': 'Create FastAPI routes and business logic',
                    'agent_id': 'builder',
                    'depends_on': [1],
                    'wave': 2,
                },
        ...
        try:
            ...
            for t in tasks_json:
                tn = t.get('task_no', 0)
                con.execute(
                    """INSERT INTO spec_tasks(spec_id,task_no,title,description,wave,depends_on,agent_id)
                               VALUES (?,?,?,?,?,?,?)""",
                    (
                        spec_id,
                        tn,
...
async def execute_spec(spec_id: str, req: Request):
    ...
    con = get_conn()
    try:
        tasks = con.execute('SELECT * FROM spec_tasks WHERE spec_id=? ORDER BY wave,task_no', (spec_id,)).fetchall()
    finally:
        con.close()
    ...
    async def _stream():
        ...
        try:
            req_msgs = [
                {
                    'role': 'system',
                    'content': 'You are a senior product architect. Generate structured requirements with EARS notation, user stories, and acceptance criteria.',
                },
        ...
        try:
            ...
            des_msgs = [
                {
                    'role': 'system',
                    'content': 'You are a senior software architect. Generate technical design with architecture, data models, API contracts, and sequence diagrams.',
                },
                {'role': 'user', 'content': f'Generate technical design for:\n\n{req_text}'},
            ]
            full_des = ''
...
Return ONLY a JSON array: [{"task_no":1,"title":"...","description":"...","wave":1,"depends_on":[],"agent_id":"builder"}]"""
            ...
            if jm:
                try:
                    tasks_json = json.loads(jm.group(0))
                except (KeyError, TypeError, ValueError, json.JSONDecodeError, OSError, AttributeError, RuntimeError):
                    pass
            if not tasks_json:
                tasks_json = [
...
def get_tasks(spec_id: str):
    """Retrieve and return get tasks."""
    from ..services.memory_db import get_conn

    con = get_conn()
    try:
        rows = con.execute('SELECT * FROM spec_tasks WHERE spec_id=? ORDER BY wave,task_no', (spec_id,)).fetchall()
    finally:
        con.close()
    ...
    try:
        tasks = [
            ...
            for r in con.execute(
                'SELECT * FROM spec_tasks WHERE spec_id=? ORDER BY wave,task_no', (spec_id,)
            ).fetchall()
        ]
    finally:
        con.close()
