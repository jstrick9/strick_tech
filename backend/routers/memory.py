...
from ..services.memory_db import (
    memory_add, memory_search_fts, memory_list,
    memory_stats, memory_galaxy_graph, get_conn, audit_log
)
...
async def hybrid_search_endpoint(q: str = "", limit: int = 20):
    ...
    from ..services.memory_db import hybrid_search, _QDRANT_AVAILABLE
...
async def add_with_embedding(req: Request):
    ...
    from ..services.memory_db import memory_add_with_vector, _ST_AVAILABLE, _QDRANT_AVAILABLE
...
async def bulk_delete(req: Request):
    ...
    try:
        from ..services.memory_db import qdrant_delete as _qd, _QDRANT_AVAILABLE
...
async def sync_all_to_qdrant():
    ...
    from ..services.memory_db import get_conn, qdrant_upsert, embed_text, _QDRANT_AVAILABLE
...
def get_memory(memory_id: int):
    ...
    try:
        row = con.execute(
            "SELECT id, source, content, tags, created_at FROM memory WHERE id=?",
            (memory_id,)
        ).fetchone()
    finally:
        con.close()
