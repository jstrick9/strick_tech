    ...
    if total > 0 and len(results) <= 5:
        try:
            ...
            res = await llm.complete(
                msgs, agent_id='free', max_tokens=100, temperature=0.3, inject_steering=False
            )  # FIX: code ops never need steering context
...
def get_project_memory(category: str = ''):
    ...
    try:
        if category:
            rows = con.execute(
                'SELECT * FROM project_memory WHERE category=? ORDER BY confidence DESC', (category,)
            ).fetchall()
        else:
...
async def learn_from_interaction(req: Request):
    ...
    messages = [
        {
            'role': 'system',
            'content': 'You extract project preferences from user actions. Return JSON array of {key, value, category} objects.\n'
...
def delete_memory(key: str):
    """Delete or remove specified memory."""
    con = memory_db.get_conn()
    try:
        cur = con.execute('DELETE FROM project_memory WHERE key=?', (key,))
    ...
    return {'ok': True, 'deleted': removed > 0, 'key': key}
...
async def get_suggestions(req: Request):
    ...
    messages = [
        {
            'role': 'system',
            'content': 'You suggest 3 smart next actions for a developer using an AI OS. '
...
def _default_suggestions(pane: str) -> list[dict]:
    ...
    defaults = {
        'chat': [
            {'label': 'Run Swarm', 'action': "nav('swarm')", 'icon': '🌀', 'reason': 'Get multiple AI perspectives'},
            {'label': 'Save to Prompts', 'action': '', 'icon': '💾', 'reason': 'Reuse this prompt later'},
            {
                'label': 'Open Studio',
                'action': "nav('studio')",
                'icon': '🎬',
                'reason': 'Build what you just discussed',
            },
        ],
        'studio': [
            {
                'label': 'Run E2E Tests',
                'action': "runE2EFull?.('web')",
                'icon': '🧪',
                'reason': 'Verify your changes work',
            },
        ...
        'kanban': [
            {'label': 'Start Pipeline', 'action': "nav('pipeline')", 'icon': '🏛️', 'reason': 'Automate this task'},
        ...
        'default': [
            {'label': 'Build in Studio', 'action': "nav('studio')", 'icon': '🎬', 'reason': 'Create something'},
...
async def review_code(req: Request):
    ...
    messages = [
        {
            'role': 'system',
            'content': f'You are a senior code reviewer for {ext} files. '
        ...
        {'role': 'user', 'content': f'Review this {ext} file ({filepath}):\n\n```{ext}\n{content[:5000]}\n```'},
    ]
