...
import json, logging, math, statistics, time, uuid
...
def _drift_score_to_color(score: float) -> str:
    if score < 10:  return "#3dba7a"
    if score < 25:  return "#5b8af8"
    if score < 45:  return "#e8a237"
    if score < 70:  return "#f06080"
    return "#e85252"
...
def compute_fingerprint(agent_id: str, window_hours: int = 168) -> dict:
    ...
    try:
        ...
        """, (agent_id, f"-{window_hours} hours")).fetchall()
    finally:
        con.close()
    ...
    try:
        con.execute("""
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (fp_id, agent_id, window_hours, now,
              lm,lp50,lp90,lp99,lstd,
              tm,tp50,tp90,tstd,
              cm,cp90,cstd,
              err_mean, tph, n))
        con.commit()
...
def compute_drift_score(agent_id: str, window_label: str = "1h") -> dict:
    ...
    try:
        fp = con.execute(
            "SELECT * FROM agent_drift_fingerprints WHERE agent_id=? AND window_hours=168",
            (agent_id,)
        ).fetchone()
        if not fp:
            return {"ok": False, "error": "No baseline fingerprint. Run /fingerprint first.",
                    "agent_id": agent_id}
        fp = dict(fp)
        ...
        """, (agent_id, f"-{hours} hours")).fetchall()
        ...
        prev = con.execute("""
            ...
            WHERE agent_id=?
              AND created_at > datetime('now', ?)
              AND created_at < datetime('now', ?)
        """, (agent_id, f"-{hours*2} hours", f"-{hours} hours")).fetchall()
        ...
        """, (agent_id, window_label)).fetchall()
    finally:
        con.close()

    if not cur:
        return {"ok": True, "agent_id": agent_id, "window_label": window_label,
                "drift_score": 0.0, "severity": "none", "trend": "insufficient_data",
                "sample_count": 0, "flags": [], "message": "No data in measurement window"}
    ...
    flags = []
    if lat_z  > 2.0: flags.append("latency_spike")
    if lat_z  > 4.0: flags.append("severe_latency")
    if tok_z  > 2.0: flags.append("token_spike")
    if tok_z  > 4.0: flags.append("token_explosion")
    if cost_z > 2.0: flags.append("cost_spike")
    if cost_z > 5.0: flags.append("cost_explosion")
    if err_z  > 2.0: flags.append("error_rate")
    if err_z  > 5.0: flags.append("reliability_drop")
    if vol_z  > 3.0: flags.append("volume_anomaly")
    if c_err_rate > 0.5: flags.append("high_failure_rate")
    ...
    elif severity == "high" and trend == "degrading":
        action = "alerted"
    ...
    try:
        cur_result = con.execute("""
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (agent_id, now, window_label,
              round(lat_z,3), round(tok_z,3), round(cost_z,3),
              round(err_z,3), round(vol_z,3),
              round(composite,2), severity, trend,
              len(cur), json.dumps(flags), action, detail[:500]))
        ...
        if severity in ("high", "critical"):
            ...
            if not existing:
                ...
                con.execute("""
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?,0,0,?,?,?)
                """, (alert_id, agent_id, score_id, severity, composite,
                      title, desc, json.dumps(flags), rec_action, now, "", ""))
                alert_created = True
                ...
                try:
                    con.execute("""
                        ...
                        VALUES (?,?,?,?,?,?,?,?)
                    """, (agent_id, "behavior_drift", severity,
                          f"Drift score {composite:.1f} — {','.join(flags)}",
                          "drift_score", composite, 0.0, now))
                except Exception:
                    pass
    ...
    log.info("Drift[%s] score=%.1f sev=%s trend=%s flags=%s",
             agent_id, composite, severity, trend, flags)
    ...
    return {
        ...
        "detail":       detail,
        "dimensions": {
            "latency":   {"zscore": round(lat_z,3),  "current": round(c_lat_mean,2),  "baseline": round(fp["lat_mean"],2),  "baseline_stddev": round(fp["lat_stddev"],2)},
            "tokens":    {"zscore": round(tok_z,3),  "current": round(c_tok_mean,2),  "baseline": round(fp["tok_mean"],2),  "baseline_stddev": round(fp["tok_stddev"],2)},
            "cost":      {"zscore": round(cost_z,3), "current": round(c_cost_mean,6), "baseline": round(fp["cost_mean"],6), "baseline_stddev": round(fp["cost_stddev"],6)},
            "error_rate":{"zscore": round(err_z,3),  "current": round(c_err_rate,4),  "baseline": round(fp["error_rate_mean"],4), "baseline_stddev": 0},
            "volume":    {"zscore": round(vol_z,3),  "current": round(c_tph,3),       "baseline": round(fp["tasks_per_hour"],3),  "baseline_stddev": 0},
        },
    }
...
def run_all_agents_drift(window_label: str = "1h") -> list[dict]:
    ...
    for a in agents:
        ...
        try:
            fp = con.execute(
                "SELECT fingerprint_id FROM agent_drift_fingerprints WHERE agent_id=?",
                (aid,)
            ).fetchone()
        finally:
            con.close()
...
def drift_summary():
    ...
    try:
        ...
        alerts_unresolved = con.execute(
            "SELECT COUNT(*) FROM drift_alerts WHERE resolved=0"
        ).fetchone()[0]
        alerts_critical   = con.execute(
            "SELECT COUNT(*) FROM drift_alerts WHERE resolved=0 AND severity='critical'"
        ).fetchone()[0]
        alerts_high       = con.execute(
            "SELECT COUNT(*) FROM drift_alerts WHERE resolved=0 AND severity='high'"
        ).fetchone()[0]

        fingerprint_count = con.execute(
            "SELECT COUNT(*) FROM agent_drift_fingerprints"
        ).fetchone()[0]

        score_count = con.execute(
            "SELECT COUNT(*) FROM agent_drift_scores"
        ).fetchone()[0]
    finally:
        con.close()
...
async def detect_agent(agent_id: str, req: Request):
    ...
    try:
        fp = con.execute(
            "SELECT computed_at FROM agent_drift_fingerprints WHERE agent_id=? AND window_hours=168",
            (agent_id,)
        ).fetchone()
    finally:
        con.close()
...
def get_agent_scores(agent_id: str, window_label: str = "", limit: int = 100):
    ...
    try:
        if window_label:
            ...
            """, (agent_id, window_label, min(limit, 500))).fetchall()
        else:
            rows = con.execute("""
                SELECT * FROM agent_drift_scores
                WHERE agent_id=?
                ORDER BY computed_at DESC LIMIT ?
            """, (agent_id, min(limit, 500))).fetchall()
    finally:
        con.close()
...
def list_alerts(
    ...
    try:
        rows = con.execute(
            f"SELECT * FROM drift_alerts WHERE {' AND '.join(where)} ORDER BY created_at DESC LIMIT ?",
            params
        ).fetchall()
    finally:
        con.close()
...
def acknowledge_alert(alert_id: str):
    ...
    try:
        ...
        con.execute(
            "UPDATE drift_alerts SET acknowledged=1, acknowledged_at=? WHERE alert_id=?",
            (_now(), alert_id)
        )
        con.commit()
    finally:
...
def resolve_alert(alert_id: str):
    ...
    try:
        ...
        if not row:
            return JSONResponse({"ok": False, "error": "Alert not found"}, status_code=404)
        con.execute(
            "UPDATE drift_alerts SET resolved=1, resolved_at=? WHERE alert_id=?",
            (_now(), alert_id)
        )
        con.commit()
    finally:
...
def drift_history(hours: int = 24, limit: int = 200):
    ...
    try:
        ...
        """, (f"-{hours} hours", min(limit, 1000))).fetchall()
    finally:
        con.close()
