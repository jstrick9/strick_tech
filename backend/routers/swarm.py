...
async def swarm_run(req: Request):
    ...
    if strategy in ("judge", "merge") and len(runs) >= 2:
        ...
        if valid_runs:
            ...
            judge_result = await llm.complete(
                judge_messages, agent_id="free", max_tokens=256, temperature=0.1,
                inject_steering=False,
            )
            try:
    ...
    if strategy == "merge" and len(runs) >= 2:
        top2 = sorted([r for r in runs if r.get("ok") and r.get("output")],
                      key=lambda r: r.get("score", 0), reverse=True)[:2]
        if len(top2) >= 2:
            merge_messages = [
            ...
            merge_result = await llm.complete(
                merge_messages, agent_id="free", max_tokens=1000, temperature=0.4,
                inject_steering=False,
            )
    ...
    try:
        ...
        try:
            ...
            VALUES (?,?,?,?,?,?,?,?,?)""",
            (run_id, prompt[:500], json.dumps(agent_ids), strategy, winner_id or "",
            winner_output[:2000], judge_reason, total_latency, total_tokens)
            )
            con.commit()
        finally:
            con.close()
