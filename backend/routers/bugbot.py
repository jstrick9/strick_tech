...
async def _run_review(diff: str, context: str = "") -> dict:
    ...
    result = await llm_svc.complete(
        ...
        agent_id="bugbot", max_tokens=3000, temperature=0.1,
...
async def review_diff(req: Request):
    ...
    try:
        ...
        (review_id, title, diff[:8000], json.dumps(issues), json.dumps([i.get("fix","") for i in issues]),
        severity, score, "done")
        )
        con.commit()
    finally:
...
async def review_diff_stream(req: Request):
    ...
    async def _stream():
        ...
        async for chunk in llm_svc.stream(
            [{"role":"system","content":REVIEW_SYSTEM},{"role":"user","content":prompt}],
            agent_id="bugbot", max_tokens=3000, inject_steering=False
        ):
...
async def review_git_diff(req: Request):
    ...
    try:
        con.execute(
        "INSERT INTO bugbot_reviews(id,title,diff,issues,fixes,severity,score,status) VALUES (?,?,?,?,?,?,?,?)",
        (review_id, f"git diff {branch or 'HEAD'}", diff[:8000],
        json.dumps(issues), json.dumps(fixes_git), review.get("severity","low"), review.get("score",75), "done")
        )
        con.commit()
    finally:
        con.close()
...
async def review_github_pr(req: Request):
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if pr_r.status_code == 403:
                return {"ok":False,"error":"GitHub rate limit exceeded or insufficient permissions. Add a GITHUB_TOKEN."}
            if pr_r.status_code == 404:
                return {"ok":False,"error":f"PR not found: {pr_url}. Check the URL and ensure the repo is accessible."}
    ...
    try:
        ...
        (review_id, pr_url, f"{owner}/{repo}", pr_num, pr_title,
        diff[:8000], json.dumps(issues), review.get("severity","low"), review.get("score",75), "done")
        )
        con.commit()
    finally:
        con.close()
...
def get_review(review_id: str):
    ...
    try:
        ...
        fb  = con.execute("SELECT * FROM bugbot_feedback WHERE review_id=? ORDER BY created_at DESC", (review_id,)).fetchall()
    finally:
        con.close()
    if not row:
