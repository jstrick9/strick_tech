...
TIER_FEATURES: dict[str, list[str]] = {
    "free": [
        "chat", "agents_basic", "kanban", "templates_basic",
        "docs", "settings", "tts_basic",
    ],
    "trial":      ["*"],   # all features during trial
    "pro": [
        "chat", "agents", "kanban", "templates", "docs", "settings",
        "tts", "swarm", "memory", "galaxy", "loops", "mcp",
        "github", "deploy", "dbstudio", "dashboard", "plugins",
        "obsidian", "system", "control", "workspaces", "webhooks",
        "testgen", "terminal", "integrations", "imagegen", "prompts",
        "codesearch", "workflow", "profiler", "pluginsdk", "multitab",
        "replay", "collabedit", "marketplace", "specs", "hooks",
        "codeindex", "arena", "steering", "bugbot", "health",
        "gitai", "ambient", "fusion", "hitl", "browser", "websearch",
        "leaderboard", "evals", "observability", "knowledge-graph", "rag",
        "voice", "pipeline",
    ],
...
def license_status():
    ...
    return {
        ...
        "trial_end_date":  datetime.fromtimestamp(data.get("trial_end", 0), tz=timezone.utc).isoformat() if is_trial else None,
...
async def activate_license(req: Request):
    ...
    if not saved:
        return {"ok": False, "error": "License activated in memory but could not be saved to disk — check file permissions."}
        ...
        "tiers": [
            {
                ...
                "features":    ["AI Chat with any model", "Basic agent creation", "Kanban board", "Template gallery", "Documentation center", "Settings & API keys"],
            ...
            },
            {
                ...
                "features":    ["Everything in Free", "All 50+ panes unlocked", "Swarm agents", "Workflow builder", "BugBot PR review", "Git AI & changelogs", "Spec-driven development", "Model Fusion", "Web Search grounding", "Voice coding"],
                "locked":      ["Agent Evals engine", "LLM Observability/DORA", "Knowledge Graph", "RAG pipeline builder", "Multi-user admin"],
                "cta":         "Upgrade to Pro",
                "highlight":   True,
            },
            {
                ...
                "features":    ["Everything in Pro", "Agent Evals (DeepEval-level)", "LLM Observability + DORA", "EU AI Act compliance", "Knowledge Graph memory", "RAG pipeline builder", "HITL governance", "Multi-user admin panel", "SOC 2 audit trails", "Priority support"],
                "locked":      [],
                "cta":         "Contact Sales",
                "highlight":   False,
            },
