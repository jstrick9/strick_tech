...
def _ensure_schema():
    ...
    try:
        ...
        for h in BUILT_IN_HOOKS:
            ...
            if not row:
                con.execute("""INSERT INTO agent_hooks(id,name,description,event,condition,prompt,agent_id,enabled)
                               VALUES (?,?,?,?,?,?,?,?)""",
                            (h["id"],h["name"],h["description"],h["event"],
                             h["condition"],h["prompt"],h["agent_id"],h["enabled"]))
        con.commit()
    finally:
        con.close()
...
async def fire_event(event_type: str, event_data: dict):
    ...
    try:
        hooks = con.execute(
            "SELECT * FROM agent_hooks WHERE event=? AND enabled=1",
            (event_type,)
        ).fetchall()
    finally:
        con.close()
    ...
    for hook in hooks:
        ...
        if h.get("condition"):
            try:
                ...
                safe_locals = {
                    "file": file_ns, "event": event_ns,
                    "commit": commit_ns, "test": test_ns,
...
async def _run_hook(hook: dict, event_data: dict):
    ...
    try:
        ...
        result = await llm_svc.complete(msgs, agent_id=hook.get("agent_id","builder"), max_tokens=1024, inject_steering=False)
    ...
    con = get_conn()
    try:
        con.execute("""INSERT INTO hook_runs(hook_id,event_data,output,status,duration_ms)
                       VALUES (?,?,?,?,?)""",
                    (hook_id, json.dumps(event_data,default=str)[:2000], output[:4000], status, dur_ms))
        con.execute("UPDATE agent_hooks SET run_count=run_count+1,last_run=CURRENT_TIMESTAMP WHERE id=?",
                    (hook_id,))
        con.commit()
    finally:
        con.close()
    ...
    try:
        ...
        await broadcast({"type":"hook_fired","hook_id":hook_id,"hook_name":hook["name"],
                         "output":output[:200],"event":hook["event"]})
    except Exception:
        pass
...
def list_hooks(event: str = "", enabled: str = ""):
    ...
    try:
        where, params = [], []
        if event:   where.append("event=?");   params.append(event)
        if enabled: where.append("enabled=?"); params.append(1 if enabled=="1" else 0)
...
async def create_hook(req: Request):
    ...
    con = get_conn()
    try:
        con.execute("""INSERT OR IGNORE INTO agent_hooks(id,name,description,event,condition,prompt,agent_id,enabled)
                       VALUES (?,?,?,?,?,?,?,?)""",
                    (hook_id,
...
def get_hook(hook_id: str):
    ...
    try:
        ...
        runs= con.execute("SELECT * FROM hook_runs WHERE hook_id=? ORDER BY created_at DESC LIMIT 10",
                          (hook_id,)).fetchall()
    finally:
        con.close()
    if not row:
