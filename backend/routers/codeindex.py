...
import ast, json, logging, os, re, time
...
def _parse_js(filepath: str, content: str) -> dict:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if m:
            symbols.append({"name":m.group(1),"type":"class","line":i,"col":0,"docstring":"","signature":line.strip()[:120],"complexity":1})
...
def _index_file(filepath: str, content: str):
    ...
    try:
        ...
        for s in parsed["symbols"]:
            con.execute("""INSERT INTO code_symbols(filepath,symbol_name,symbol_type,line_no,col_no,docstring,signature,complexity)
                           VALUES (?,?,?,?,?,?,?,?)""",
                        (rel_path,s["name"],s["type"],s["line"],s.get("col",0),
                         s.get("docstring",""),s.get("signature",""),s.get("complexity",1)))
        for imp in parsed["imports"]:
            con.execute("INSERT INTO code_imports(from_file,to_module,import_name,line_no) VALUES (?,?,?,?)",
                        (rel_path,imp["to_module"],imp.get("import_name",""),imp.get("line",0)))
        for call in parsed["calls"]:
            con.execute("INSERT INTO code_calls(from_file,from_symbol,to_symbol,line_no) VALUES (?,?,?,?)",
...
def search_symbols(q: str = "", file: str = "", type: str = "", limit: int = 50):
    ...
    try:
        where, params = [], []
        if file: where.append("filepath LIKE ?"); params.append(f"%{file}%")
        if type: where.append("symbol_type=?");   params.append(type)
        if q:
            where.append("symbol_name LIKE ?");   params.append(f"%{q}%")
        ...
        total = con.execute(
            f"SELECT COUNT(*) FROM code_symbols{count_where}", count_params
        ).fetchone()[0]
    finally:
        con.close()
...
def dependency_graph(limit: int = 200):
    ...
    try:
        ...
        imports = con.execute(
            "SELECT from_file, to_module FROM code_imports LIMIT ?",
            (limit*5,)
        ).fetchall()
    finally:
        con.close()
    ...
    nodes    = [{"id":r["filepath"],"name":Path(r["filepath"]).name,
                 "size":r["sym_count"],"type":"file"} for r in files]
...
def file_symbols(filepath: str):
    ...
    try:
        symbols = con.execute(
            "SELECT * FROM code_symbols WHERE filepath LIKE ? ORDER BY line_no",
            (f"%{filepath}%",)
        ).fetchall()
        imports = con.execute(
            "SELECT * FROM code_imports WHERE from_file LIKE ? ORDER BY line_no",
            (f"%{filepath}%",)
        ).fetchall()
        calls   = con.execute(
            "SELECT * FROM code_calls WHERE from_file LIKE ? ORDER BY line_no",
            (f"%{filepath}%",)
        ).fetchall()
    finally:
        con.close()
...
def find_references(symbol_name: str):
    ...
    try:
        calls   = con.execute(
            "SELECT * FROM code_calls WHERE to_symbol=? ORDER BY from_file,line_no",
            (symbol_name,)
        ).fetchall()
        defined = con.execute(
...
def dead_code_detection():
    ...
    dead = [{"symbol_name":r["symbol_name"],"filepath":r["filepath"]}
            for r in defined
...
def index_stats():
    ...
    try:
        ...
        avg_complexity= con.execute("SELECT AVG(complexity) FROM code_symbols WHERE symbol_type='function'").fetchone()[0]
    finally:
        con.close()
    return {
