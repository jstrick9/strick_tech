...
ROLE_DEFAULTS: dict[str, dict] = {
    "developer": {"pinned_panes": ["chat","studio","github","terminal","bugbot","codeindex"],    "quick_actions": ["chat","studio","bugbot","gitai"],        "default_agent": "builder"},
    "analyst":   {"pinned_panes": ["chat","dashboard","dbstudio","rag","evals"],                 "quick_actions": ["chat","rag","evals","dashboard"],        "default_agent": "researcher"},
    "writer":    {"pinned_panes": ["chat","prompts","templates","docs"],                          "quick_actions": ["chat","prompts","templates","websearch"],"default_agent": "builder"},
    "designer":  {"pinned_panes": ["chat","imagegen","studio","templates"],                       "quick_actions": ["chat","imagegen","studio","collabedit"], "default_agent": "builder"},
    "manager":   {"pinned_panes": ["chat","kanban","dashboard","control","leaderboard"],          "quick_actions": ["chat","kanban","dashboard","ambient"],   "default_agent": "orchestrator"},
    "student":   {"pinned_panes": ["chat","docs","templates","kanban"],                           "quick_actions": ["chat","docs","templates","arena"],       "default_agent": "builder"},
}
...
async def update_profile(req: Request):
    ...
    if "ui_mode" in body and body["ui_mode"] not in VALID_UI_MODES:
        return JSONResponse(status_code=422, content={"ok": False, "error": f"ui_mode must be one of: {sorted(VALID_UI_MODES)}"})
    if "theme" in body and body["theme"] not in VALID_THEMES:
        return JSONResponse(status_code=422, content={"ok": False, "error": f"theme must be one of: {sorted(VALID_THEMES)}"})
    if "font_size" in body and body["font_size"] not in VALID_FONT_SIZES:
        return JSONResponse(status_code=422, content={"ok": False, "error": f"font_size must be one of: {sorted(VALID_FONT_SIZES)}"})
    if "skill_level" in body and body["skill_level"] not in VALID_SKILL_LEVELS:
        return JSONResponse(status_code=422, content={"ok": False, "error": f"skill_level must be one of: {sorted(VALID_SKILL_LEVELS)}"})

    updatable = {
        "name", "email", "avatar", "role", "skill_level", "ui_mode", "theme",
        "font_size", "sidebar_width", "show_tips", "show_tour", "onboarding_done",
        "onboarding_step", "default_agent", "default_model",
        "hidden_panes", "pinned_panes", "sidebar_order",
        "enabled_features", "quick_actions", "notifications",
    }
    for k in updatable:
        if k not in body:
            continue
        if k == "name":            profile[k] = str(body[k])[:100]
        elif k == "email":         profile[k] = str(body[k])[:200]
        elif k == "avatar":        profile[k] = str(body[k])[:10]
    ...
    saved = _save(profile)
    if not saved:
        return JSONResponse(status_code=500, content={"ok": False, "error": "Profile updated in memory but could not persist to disk"})
...
async def complete_onboarding(req: Request):
    ...
    if body.get("name"):  profile["name"] = str(body["name"])[:100]
...
def get_ui_config():
    ...
    profile = _load()
    try:
        from .license import _load_license, _effective_tier, _days_remaining
        lic       = _load_license()
