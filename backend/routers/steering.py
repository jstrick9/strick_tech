...
def _ensure_schema():
    ...
    try:
        ...
        for sf in STARTER_FILES:
            con.execute(
                "UPDATE steering_files SET enabled=1 WHERE id=? AND enabled=0 AND auto_learned=0",
                (sf["id"],)
            )
        con.commit()
    finally:
...
def list_steering():
    ...
    try:
        rows = con.execute(
            "SELECT * FROM steering_files ORDER BY category, title"
        ).fetchall()
    finally:
        con.close()
...
async def update_steering(file_id: str, req: Request):
    ...
    try:
        ...
        if title:               sets.append("title=?");   vals.append(title[:120])
...
async def learn_from_chat(req: Request):
    ...
    try:
        msgs = con.execute(
            "SELECT role, message FROM chat_log WHERE role='user' ORDER BY created_at DESC LIMIT ?",
            (min(limit, 200),)
        ).fetchall()
    finally:
        con.close()
...
Return ONLY valid JSON. Use null for unknown fields."""
    ...
    result = await llm_svc.complete(
        ...
        agent_id="steering", max_tokens=1000, temperature=0.1, inject_steering=False
    )
    ...
    EXPECTED_KEYS = {"preferred_language","preferred_framework","coding_style",
                     "naming_conventions","error_handling","testing_preferences",
                     "architecture_patterns","avoid_patterns","custom_rules"}
    ...
    try:
        ...
        for key, val in learned.items():
            ...
            try:
                existing = con.execute("SELECT occurrences FROM steering_learned WHERE pattern_key=?", (key,)).fetchone()
                if existing:
                    con.execute(
...
async def promote_learned_to_steering(req: Request):
    ...
    try:
        patterns = con.execute(
            "SELECT * FROM steering_learned WHERE confidence>=? AND promoted=0 ORDER BY confidence DESC",
            (min_conf,)
        ).fetchall()
    finally:
        con.close()
...
def get_learned_patterns():
    ...
    try:
        rows = con.execute(
            "SELECT * FROM steering_learned ORDER BY confidence DESC, occurrences DESC"
        ).fetchall()
    finally:
        con.close()
