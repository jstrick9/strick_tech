...
def _retrieve_chunks(pipeline_id: str, query: str, k: int) -> list[dict]:
    ...
    try:
        ...
        try:
            ...
            rows = con.execute("""
                ...
                ORDER BY rank LIMIT ?
            """, (fts_query, pipeline_id, min(k, 20))).fetchall()
        except Exception:
            rows = con.execute(
    ...
    return [{"chunk_id": r["id"], "content": r["content"],
             "chunk_no": r["chunk_no"], "doc_id": r["doc_id"]} for r in rows]
...
async def create_pipeline(req: Request):
    ...
    con = get_conn()
    try:
        con.execute("INSERT INTO rag_pipelines(id,name,description,chunk_strategy,chunk_size,chunk_overlap,retrieval_k) VALUES (?,?,?,?,?,?,?)",
        (pid, body.get("name","RAG Pipeline"), body.get("description",""),
        body.get("chunk_strategy","paragraph"), int(body.get("chunk_size",500)),
        int(body.get("chunk_overlap",50)), int(body.get("retrieval_k",5))))
        con.commit()
    finally:
        con.close()
...
def get_pipeline(pipeline_id: str):
    ...
    try:
        ...
        docs = con.execute("SELECT * FROM rag_documents WHERE pipeline_id=? ORDER BY created_at", (pipeline_id,)).fetchall()
    finally:
        con.close()
    if not pipe: return {"ok":False,"error":"Not found"}
...
async def add_document(pipeline_id: str, req: Request):
    ...
    try:
        ...
        con.close()
    if not pipe: return {"ok":False,"error":"Pipeline not found"}
    p = dict(pipe)
    ...
    con = get_conn()
    try:
        con.execute("INSERT INTO rag_documents(id,pipeline_id,filename,content_hash,char_count,chunk_count) VALUES (?,?,?,?,?,?)",
                    (doc_id, pipeline_id, filename, chash, len(content), len(chunks)))
        # Store chunks
        for i, chunk in enumerate(chunks):
            cid = f"chk_{uuid.uuid4().hex[:6]}"
            con.execute("INSERT INTO rag_chunks(id,pipeline_id,doc_id,chunk_no,content) VALUES (?,?,?,?,?)",
        ...
        con.execute("UPDATE rag_pipelines SET doc_count=doc_count+1, chunk_count=chunk_count+?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
...
async def rag_query(pipeline_id: str, req: Request):
    ...
    if not chunks:
        return {"ok":True,"answer":"No relevant documents found in this pipeline.","citations":[],"chunks_used":0}
...
Return JSON: {{"faithfulness": 0.0-1.0, "unsupported_claims": ["list any claims not in context"]}}"""
    ...
    faith_r = await llm_svc.complete([{"role":"user","content":faith_prompt}], agent_id="evals", max_tokens=200, temperature=0.1, inject_steering=False)
    faith_d = {}
    m = re.search(r'\{.*\}', faith_r.get("text",""), re.DOTALL)
    if m:
        try: faith_d = json.loads(m.group(0))
...
Return JSON: {{"relevancy": 0.0-1.0, "reason": "brief"}}"""
    ...
    rel_r = await llm_svc.complete([{"role":"user","content":rel_prompt}], agent_id="evals", max_tokens=150, temperature=0.1, inject_steering=False)
    rel_d = {}
    m = re.search(r'\{.*\}', rel_r.get("text",""), re.DOTALL)
    if m:
        try: rel_d = json.loads(m.group(0))
        except Exception: pass
...
def list_documents(pipeline_id: str):
    ...
    con = get_conn()
    try:
        rows = con.execute("SELECT * FROM rag_documents WHERE pipeline_id=? ORDER BY created_at", (pipeline_id,)).fetchall()
    finally:
        con.close()
...
def delete_document(pipeline_id: str, doc_id: str):
    ...
    con = get_conn()
    try:
        doc = con.execute("SELECT chunk_count FROM rag_documents WHERE id=? AND pipeline_id=?", (doc_id,pipeline_id)).fetchone()
        if doc:
