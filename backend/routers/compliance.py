...
def _collect_report_data(date_from: str, date_to: str, scope: dict) -> dict:
    ...
    try:
        ...
        if scope.get('audit_chain', True):
            ...
            by_risk = con.execute(
                f'SELECT risk_level, COUNT(*) cnt FROM audit_log_chain {where_time_epoch} GROUP BY risk_level',
                params_epoch,
            ).fetchall()
            by_out = con.execute(
                f'SELECT outcome, COUNT(*) cnt FROM audit_log_chain {where_time_epoch} GROUP BY outcome', params_epoch
            ).fetchall()
            by_agent = con.execute(
                f'SELECT agent_id, agent_name, COUNT(*) cnt FROM audit_log_chain {where_time_epoch} GROUP BY agent_id ORDER BY cnt DESC LIMIT 15',
                params_epoch,
            ).fetchall()
            ...
            if where_time_epoch:
                ...
                fail_sql = (
                    f"SELECT * FROM audit_log_chain {where_time_epoch} AND outcome='failure' ORDER BY seq DESC LIMIT 50"
                )
                hr_params = params_epoch
            else:
                hr_sql = (
                    "SELECT * FROM audit_log_chain WHERE risk_level IN ('high','critical') ORDER BY seq DESC LIMIT 50"
                )
            ...
            recent_entries = con.execute(
                f'SELECT seq,entry_id,agent_id,agent_name,action_type,action_detail,risk_level,outcome,created_at,entry_hash FROM audit_log_chain {where_time_epoch} ORDER BY seq DESC LIMIT 200',
                params_epoch,
            ).fetchall()
        ...
        if scope.get('hitl', True):
            ...
            hitl_status = con.execute(
                f'SELECT status, COUNT(*) cnt FROM hitl_queue {where_time} GROUP BY status', params_time
            ).fetchall()
            hitl_items = con.execute(
                f'SELECT * FROM hitl_queue {where_time} ORDER BY created_at DESC LIMIT 100', params_time
            ).fetchall()
            hitl_audit = con.execute(
                f'SELECT * FROM hitl_audit {where_time} ORDER BY created_at DESC LIMIT 100', params_time
            ).fetchall()
            data['hitl'] = {
                'total': hitl_total,
        ...
        if scope.get('policies', True):
            ...
            pol_decision = con.execute(
                f'SELECT policy_decision, COUNT(*) cnt FROM mcp_gateway_calls {where_time} GROUP BY policy_decision',
                params_time,
            ).fetchall()
            ...
            active_policies = con.execute(
                'SELECT policy_id, name, action, agent_id, server_id, tool_pattern, priority FROM mcp_gateway_policies WHERE enabled=1 ORDER BY priority'
            ).fetchall()
            data['policies'] = {
        ...
        if scope.get('agent_identity', True):
            agents = con.execute(
                'SELECT agent_id, display_name, status, authority_level, key_version, created_at FROM agent_identities ORDER BY created_at DESC LIMIT 50'
            ).fetchall()
        ...
        if scope.get('connectors', True):
            ...
            ce_status = con.execute(
                f'SELECT status, COUNT(*) cnt FROM connector_executions {where_time} GROUP BY status', params_time
            ).fetchall()
            ce_by_conn = con.execute(
                f'SELECT connector_id, COUNT(*) cnt FROM connector_executions {where_time} GROUP BY connector_id ORDER BY cnt DESC',
                params_time,
            ).fetchall()
            if where_time:
        ...
        if scope.get('cost', True):
            try:
                cost_total = con.execute(
                    f'SELECT SUM(cost_usd), SUM(total_tokens) FROM cost_ledger {where_time}', params_time
                ).fetchone()
                cost_by_agent = con.execute(
                    f'SELECT agent_id, SUM(cost_usd) total_cost, SUM(total_tokens) total_tokens FROM cost_ledger {where_time} GROUP BY agent_id ORDER BY total_cost DESC LIMIT 15',
                    params_time,
                ).fetchall()
                cost_by_model = con.execute(
                    f'SELECT model, SUM(cost_usd) total_cost, COUNT(*) calls FROM cost_ledger {where_time} GROUP BY model ORDER BY total_cost DESC LIMIT 10',
                    params_time,
                ).fetchall()
                budget_caps = con.execute('SELECT * FROM budget_caps').fetchall()
                cost_alerts = con.execute(
                    f'SELECT * FROM cost_alerts {where_time} ORDER BY created_at DESC LIMIT 20', params_time
                ).fetchall()
                data['cost'] = {
        ...
        if scope.get('supervisor', True):
            ...
            sv_status = con.execute(
                f'SELECT status, COUNT(*) cnt FROM supervisor_runs {where_time} GROUP BY status', params_time
            ).fetchall()
            sv_runs = con.execute(
                f'SELECT run_id,goal_title,status,task_count,done_count,eval_score,total_tokens,total_cost,duration_ms,created_at FROM supervisor_runs {where_time} ORDER BY created_at DESC LIMIT 50',
                params_time,
            ).fetchall()
            data['supervisor'] = {
                'total': sv_total,
...
def _generate_pdf(report_data: dict, title: str, framework: str) -> bytes:
    ...
    class CompliancePDF(FPDF):
        """Data structure or service class representing CompliancePDF."""

        def header(self):
            # Dark header bar
            """Execute or process header operation."""
        ...
        def footer(self):
            """Execute or process footer operation."""
            self.set_y(-15)
            ...
            self.cell(
                0,
                10,
                f'Page {self.page_no()} - {report_data.get("generated_at", "")[:19]} UTC - CONFIDENTIAL',
                align='C',
            )
    ...
    def h1(text):
        """Execute or process h1 operation."""
    ...
    def h2(text, color=None):
        """Execute or process h2 operation."""
    ...
    def body(text, size=9, color=None):
        """Execute or process body operation."""
    ...
    def stat_box(label, value, color):
        """Execute or process stat box operation."""
    ...
    def section_divider(color=None):
        """Execute or process section divider operation."""
        c = color or GRAY_MED
    ...
    def risk_badge(risk_level: str) -> str:
        """Execute or process risk badge operation."""
        return {'low': 'LOW', 'medium': 'MED', 'high': 'HIGH', 'critical': 'CRIT'}.get(
            risk_level, risk_level.upper()[:4]
        )
    ...
    meta = [
        ...
        (
            'Chain Integrity',
            escHtml_py(str(report_data.get('audit', {}).get('chain_integrity', {}).get('message', 'Not verified'))),
        ),
    ...
    if report_data.get('audit'):
        sections.append(('1', 'Audit Chain Integrity & Decision Trail'))
    if report_data.get('hitl'):
        sections.append(('2', 'Human-in-the-Loop Approvals'))
    if report_data.get('policies'):
        sections.append(('3', 'Policy Enforcement Summary'))
    if report_data.get('agent_identity'):
        sections.append(('4', 'Agent Identity & Access Records'))
    if report_data.get('connectors'):
        sections.append(('5', 'Connector Executions'))
    if report_data.get('cost'):
        sections.append(('6', 'Cost & Token Attribution'))
    if report_data.get('supervisor'):
        sections.append(('7', 'Supervisor Run Outcomes'))
    ...
    if report_data.get('audit'):
        ...
        body(
            escHtml_py(
                f'Verified entries: {ci.get("verified", 0):,}   Chain tip: {(ci.get("chain_tip", ""))[:32]}...  {str(ci.get("message", ""))}'
            )
        )
        if not ok:
            body(
                '[!] ATTENTION: Chain broken at seq='
                + str(ci.get('broken_at', '?'))
                + '. This may indicate tampering. Immediate investigation required.',
                color=BRAND_RED,
            )
        pdf.ln(4)
        ...
        stat_box(
            'Failures', audit.get('failure_count', 0), BRAND_RED if audit.get('failure_count', 0) > 0 else GRAY_MED
        )
        stat_box(
            'High Risk',
            audit.get('high_risk_count', 0),
            BRAND_AMBER if audit.get('high_risk_count', 0) > 0 else GRAY_MED,
        )
        ...
        if audit.get('high_risk'):
            ...
            pdf.set_font('Helvetica', '', 8)
            body(
                f'The following {len(audit["high_risk"])} entries were flagged as high or critical risk during the report period:'
            )
            pdf.ln(2)
            ...
            if len(audit['high_risk']) > 30:
                body(
                    f'... and {len(audit["high_risk"]) - 30} more. See JSON export for full list.',
                    color=(150, 150, 180),
                )
            pdf.ln(4)
    ...
    if report_data.get('hitl'):
        ...
        for status, cnt in hitl.get('by_status', {}).items():
            sc = {'approved': BRAND_GREEN, 'rejected': BRAND_RED, 'pending': BRAND_AMBER, 'timeout': GRAY_MED}.get(
                status, GRAY_MED
            )
            pdf.set_font('Helvetica', '', 8)
        ...
        if hitl.get('items'):
            ...
            for item in hitl['items'][:20]:
                ...
                pdf.set_font('Helvetica', '', 8)
                pdf.set_text_color(80, 85, 120)
                status_c = {'approved': BRAND_GREEN, 'rejected': BRAND_RED, 'pending': BRAND_AMBER}.get(
                    item_d.get('status', ''), GRAY_MED
                )
                pdf.set_font('Helvetica', 'B', 8)
                ...
                pdf.cell(
                    0,
                    4,
                    escHtml_py(str(item_d.get('action_type', '') or item_d.get('interrupt_type', ''))[:60]),
                    new_x=XPos.LMARGIN,
                    new_y=YPos.NEXT,
                )
    ...
    if report_data.get('policies'):
        ...
        for p in pol['active_policies'][:20]:
            ac = {'allow': BRAND_GREEN, 'deny': BRAND_RED, 'require_hitl': BRAND_AMBER}.get(
                p.get('action', ''), GRAY_MED
            )
            pdf.set_font('Helvetica', '', 7)
            pdf.set_text_color(100, 110, 150)
        ...
        if cost.get('budget_caps'):
            ...
            for cap in cost['budget_caps'][:10]:
                ...
                pdf.cell(
                    0,
                    4,
                    f'Max ${cap_d.get("max_cost_usd", 0):.2f}/day   {cap_d.get("max_tokens", 0):,} tokens/day',
                    new_x=XPos.LMARGIN,
                    new_y=YPos.NEXT,
                )
        ...
        if sv.get('runs'):
            ...
            for r in sv['runs'][:20]:
                ...
                sc = (
                    ...
                    if rd.get('status') in ('failed', 'killed')
                    else BRAND_AMBER
                )
                pdf.set_font('Helvetica', '', 7)
                pdf.set_text_color(80, 85, 120)
    ...
    body(
        "This certificate attests that the audit chain has been cryptographically verified using SHA-256 hash chaining. Each entry's hash is computed from all critical fields plus the previous entry's hash, forming a tamper-evident chain. Any modification to any entry would invalidate all subsequent hashes and be immediately detectable upon verification.",
        size=8,
        color=(80, 90, 120),
    )
        ...
        'frameworks': [
            {
                'id': 'General',
                'name': 'General Audit',
                'description': 'Comprehensive audit trail for general governance review',
            },
            {
                'id': 'SOC2',
                'name': 'SOC 2 Type II',
                'description': 'Security, Availability, Confidentiality, Processing Integrity, Privacy',
            },
            {
                'id': 'GDPR',
                'name': 'GDPR',
                'description': 'Article 30 records of processing, Article 32 security measures',
            },
            {
                'id': 'HIPAA',
                'name': 'HIPAA',
                'description': 'Security Rule §164.312 technical safeguards and audit controls',
            },
            {
                'id': 'FINRA',
                'name': 'FINRA',
                'description': 'Rule 4370 continuity planning, Rule 17a-4 record retention',
            },
            {
                'id': 'ISO27001',
                'name': 'ISO/IEC 27001',
                'description': 'Annex A technology and organizational controls documentation',
            },
        ]
    }
...
async def generate_report(req: Request):
    ...
    try:
        con.execute(
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?)
        """,
            (report_id, title, framework, date_from, date_to, fmt, json.dumps(scope), 'generating', 'user', now),
        )
        con.commit()
    finally:
        con.close()
    ...
    try:
        ...
        summary = {
            ...
            'policy_blocked': sum(
                v for k, v in data.get('policies', {}).get('by_decision', {}).items() if k in ('deny', 'require_hitl')
            ),
...
def compliance_summary():
    ...
    try:
        ...
        high_risk = con.execute(
            "SELECT COUNT(*) FROM audit_log_chain WHERE risk_level IN ('high','critical')"
        ).fetchone()[0]
        ...
        policy_blocked = con.execute(
            "SELECT COUNT(*) FROM mcp_gateway_calls WHERE policy_decision IN ('deny','require_hitl')"
        ).fetchone()[0]
        ...
        last_report = con.execute(
            "SELECT created_at, framework FROM compliance_reports WHERE status='done' ORDER BY created_at DESC LIMIT 1"
        ).fetchone()
    finally:
        con.close()
