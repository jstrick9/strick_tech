...
import asyncio, base64, json, logging, os, re, time, uuid
...
async def _do_generate(prompt: str, size: str = "1024x1024",
                       style: str = "", save_to: str = "") -> dict:
...
def list_models():
    ...
    return {
        "models": [
            {"id": "black-forest-labs/FLUX.1-schnell:free", "name": "Flux Schnell (Free)", "provider": "OpenRouter", "free": True,  "quality": "fast"},
            {"id": "black-forest-labs/FLUX.1-pro",          "name": "Flux Pro",             "provider": "OpenRouter", "free": False, "quality": "high"},
            {"id": "openai/dall-e-3",                       "name": "DALL-E 3",              "provider": "OpenAI",     "free": False, "quality": "high"},
            {"id": "stability-ai/sdxl",                     "name": "Stable Diffusion XL",  "provider": "Stability",  "free": False, "quality": "medium"},
        ],
...
async def inject_image_into_code(req: Request):
    ...
    for desc in placeholders:
        ...
        result = await _do_generate(
            prompt=desc,
            save_to=f"assets/images/{safe_name}.svg"
        )
        if result.get("ok"):
            ...
            if img_src:
                content = content.replace(
                    placeholder,
                    f'<img src="{img_src}" alt="{desc}" style="max-width:100%;border-radius:8px">',
                    1
                )
                injected += 1
...
async def figma_import(req: Request):
    ...
    messages = [
        {"role": "system", "content":
            f"You are a UI developer converting a Figma design to {framework} code. "
        ...
        {"role": "user", "content":
            f"Convert this Figma design to {framework}:\n{figma_url}\n\n"
            f"Generate a complete, pixel-perfect reconstruction based on the design name/context."}
    ]
    result = await llm.complete(messages, agent_id="builder", max_tokens=4096,
                                temperature=0.3, inject_steering=False)
...
def image_styles():
    ...
    return [
        {"id": "photorealistic", "label": "📸 Photorealistic", "prompt": "photorealistic, 8k, detailed, natural lighting"},
        ...
        {"id": "cinematic",      "label": "🎬 Cinematic",      "prompt": "cinematic lighting, film grain, dramatic shadows, movie still"},
        {"id": "watercolor",     "label": "🎨 Watercolor",     "prompt": "watercolor painting, soft edges, paper texture, translucent washes"},
        {"id": "pixel_art",      "label": "👾 Pixel Art",      "prompt": "pixel art, 16-bit, retro game style, chunky pixels"},
        {"id": "sketch",         "label": "✏️ Sketch",          "prompt": "pencil sketch, hand-drawn, graphite, cross-hatching, monochrome"},
    ]
...
async def style_transfer(req: Request):
    ...
    if key and source_prompt:
        try:
            ...
            msgs = [
                {"role": "system", "content": "You are an expert image generation prompt engineer. Enhance prompts for maximum visual quality. Return ONLY the enhanced prompt, no explanation."},
                {"role": "user",   "content": f"Enhance this image prompt with {style_id} style:\n{enhanced_prompt}\n\nReturn an optimized prompt for Flux image generation, max 200 words."}
            ]
            result          = await llm_svc.complete(msgs, agent_id="imagegen", max_tokens=256,
                                                     temperature=0.7, inject_steering=False)
...
async def enhance_prompt(req: Request):
    ...
    if key:
        try:
            from ..services import llm as llm_svc
            msgs = [
                {"role": "system", "content": "You are an expert at writing image generation prompts for Flux/DALL-E/Midjourney. Make prompts vivid, specific, and technically optimized. Return ONLY the enhanced prompt."},
            ...
            r        = await llm_svc.complete(msgs, agent_id="imagegen", max_tokens=200,
                                              temperature=0.8, inject_steering=False)
