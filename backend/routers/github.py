        ...
        for file_path in file_paths[:100]:  # limit to 100 files
            ...
            try:
                ...
                check = await client.get(
                    f'{GITHUB_API}/repos/{repo_name}/contents/{rel_path}', headers=_gh_headers(), params={'ref': branch}
                )
                ...
                r = await client.put(
                    f'{GITHUB_API}/repos/{repo_name}/contents/{rel_path}', headers=_gh_headers(), json=payload
                )
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=20.0) as client:
            for blob in blobs[:100]:
                ...
                if any(
                    path.endswith(ext)
                    for ext in ['.html', '.css', '.js', '.jsx', '.ts', '.tsx', '.json', '.md', '.svg', '.txt']
                ):
...
async def create_branch(owner: str, repo: str, req: Request):
    ...
    try:
        ...
        # Create new branch
        await _gh_post(f'/repos/{owner}/{repo}/git/refs', {'ref': f'refs/heads/{branch_name}', 'sha': sha})
        return {
            'ok': True,
            'branch': branch_name,
            'from': from_branch,
...
async def create_pr(owner: str, repo: str, req: Request):
    ...
    try:
        pr = await _gh_post(
            f'/repos/{owner}/{repo}/pulls', {'title': title, 'head': head, 'base': base, 'body': body_text}
        )
        from ..services.memory_db import audit_log
...
async def deploy_github_pages(req: Request):
    ...
    push_result = await push_to_github(
        _make_internal_request({'repo': repo_name, 'branch': 'gh-pages', 'message': message})
    )
    ...
    if not push_result.get('ok'):
        ...
        try:
            ...
            await _gh_post(f'/repos/{repo_name}/git/refs', {'ref': 'refs/heads/gh-pages', 'sha': sha})
    ...
    with contextlib.suppress(Exception):
        await _gh_post(f'/repos/{repo_name}/pages', {'source': {'branch': 'gh-pages', 'path': '/'}})  # May already be enabled
...
async def sync_with_github(req: Request):
    ...
    if direction in ('push', 'both'):
        results['push'] = await push_to_github(
            _make_internal_request(
                {
                    'repo': repo_name,
                    'branch': branch,
                    'message': f'Auto-sync from Agentic OS {time.strftime("%Y-%m-%d %H:%M")}',
                }
            )
        )
    if direction in ('pull', 'both'):
        results['pull'] = await pull_from_github(_make_internal_request({'repo': repo_name, 'branch': branch}))
