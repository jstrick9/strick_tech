...
import json, logging, time, uuid, asyncio, os
...
STARTER_WORKFLOWS = [
    {
        ...
        "nodes": [
            ...
            {"id":"n3","type":"agent","label":"Summarizer","x":540,"y":200,"config":{"agent_id":"builder","prompt":"Summarize this research in 3 bullets:\n{{prev_output}}"}},
    ...
    },
    {
        ...
        "nodes": [
            {"id":"n1","type":"trigger","label":"Webhook / Schedule","x":60,"y":180,"config":{"event":"webhook"}},
            {"id":"n2","type":"agent","label":"Code Gen","x":280,"y":180,"config":{"agent_id":"builder","prompt":"Generate code for: {{input}}"}},
            {"id":"n3","type":"agent","label":"Code Review","x":500,"y":100,"config":{"agent_id":"researcher","prompt":"Review this code:\n{{prev_output}}"}},
            {"id":"n4","type":"agent","label":"Test Gen","x":500,"y":260,"config":{"agent_id":"builder","prompt":"Write tests for:\n{{prev_output}}"}},
            {"id":"n5","type":"condition","label":"Tests Pass?","x":720,"y":180,"config":{"expression":"{{prev_output}} contains 'pass'"}},
            {"id":"n6","type":"output","label":"Deploy","x":940,"y":140,"config":{"target":"deploy"}},
            {"id":"n7","type":"output","label":"Notify Fail","x":940,"y":260,"config":{"target":"notification"}},
        ],
        "edges": [
            {"id":"e1","from":"n1","to":"n2"},
            {"id":"e2","from":"n2","to":"n3"},
    ...
    },
    {
        ...
        "nodes": [
            ...
            {"id":"n4","type":"agent","label":"Agent C","x":280,"y":320,"config":{"agent_id":"orchestrator"}},
            {"id":"n5","type":"transform","label":"Merge & Judge","x":520,"y":200,"config":{"mode":"merge"}},
            {"id":"n6","type":"output","label":"Final Answer","x":740,"y":200,"config":{"target":"chat"}},
        ],
        "edges": [
            {"id":"e1","from":"n1","to":"n2"},{"id":"e2","from":"n1","to":"n3"},{"id":"e3","from":"n1","to":"n4"},
            {"id":"e4","from":"n2","to":"n5"},{"id":"e5","from":"n3","to":"n5"},{"id":"e6","from":"n4","to":"n5"},
            {"id":"e7","from":"n5","to":"n6"},
        ],
        "created_at": "2025-01-01T00:00:00Z",
    },
...
def node_types():
    return {"types": [
        {"id":"trigger",   "label":"⚡ Trigger",    "color":"#5b8af8","desc":"Start the workflow (webhook, schedule, chat, manual)","inputs":0,"outputs":1},
        {"id":"agent",     "label":"🤖 Agent",      "color":"#9d74f5","desc":"Run an AI agent with a prompt","inputs":1,"outputs":1},
        {"id":"condition", "label":"🔀 Condition",  "color":"#e8a237","desc":"Branch based on output content","inputs":1,"outputs":2},
        {"id":"transform", "label":"⚙️ Transform",  "color":"#38c5d8","desc":"Merge, filter, or format data","inputs":-1,"outputs":1},
        {"id":"loop",      "label":"🔁 Loop",       "color":"#f06080","desc":"Repeat N times or until condition","inputs":1,"outputs":1},
        {"id":"delay",     "label":"⏱️ Delay",      "color":"#7a8aaa","desc":"Wait N seconds before next node","inputs":1,"outputs":1},
        {"id":"webhook",   "label":"🌐 Webhook",    "color":"#4cc98a","desc":"Call an external API","inputs":1,"outputs":1},
        {"id":"output",    "label":"📤 Output",     "color":"#3dba7a","desc":"Send to chat, deploy, notification, or file","inputs":1,"outputs":0},
        {"id":"memory",    "label":"🧠 Memory",     "color":"#c084fc","desc":"Read/write to agent memory","inputs":1,"outputs":1},
        {"id":"code",      "label":"</> Code",      "color":"#f08850","desc":"Run custom JavaScript transform","inputs":1,"outputs":1},
    ]}
    ...
    async def _stream():
        ...
        while queue:
            ...
            if node["type"] == "trigger":
                ...
                prompt = prompt.replace("{{input}}", context["input"]).replace("{{prev_output}}", context["prev_output"])
                
                try:
                    msgs = [{"role":"user","content":prompt}]
                    result = await llm_svc.complete(msgs, agent_id=cfg.get("agent_id","builder"), max_tokens=1024, inject_steering=False)
