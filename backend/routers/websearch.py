...
import asyncio, json, logging, re, time, sqlite3
...
async def _ddg_search(query: str, num_results: int = 5) -> list[dict]:
    ...
    try:
        ...
        async with httpx.AsyncClient(
            timeout=10, headers={"User-Agent": "Mozilla/5.0 AgenticOS/6.0"}
        ) as client:
            r = await client.get(url)
            if r.status_code == 200:
                text = r.text
                link_pattern    = re.compile(
                    r'<a[^>]+href="(https?://[^"]+)"[^>]*>([^<]+)</a>', re.IGNORECASE
                )
...
async def _fetch_page_text(url: str, max_chars: int = 2000) -> str:
    ...
    try:
        async with httpx.AsyncClient(
            timeout=8, headers={"User-Agent": "Mozilla/5.0 AgenticOS/6.0"}
        ) as client:
...
async def grounded_stream(req: Request):
    ...
    async def _stream():
        ...
        citations = [
            {"num": i + 1, "title": res["title"], "url": res["url"]}
...
async def deep_research(req: Request):
    ...
    async def _stream():
        ...
        qgen = await llm_svc.complete(
            ...
            agent_id="research", max_tokens=200, temperature=0.5, inject_steering=False,
        )
        ...
        async for chunk in llm_svc.stream(
            ...
            agent_id="research", max_tokens=3000, inject_steering=False,
        ):
            yield f"data: {json.dumps({'type': 'chunk', 'text': chunk})}\n\n"

        citations = [
            {"num": i + 1, "title": s["title"], "url": s["url"]}
            for i, s in enumerate(unique_sources[:15])
        ]
...
async def get_history(limit: int = 50):
    ...
    try:
        rows = con.execute(
            "SELECT id, query, kind, results, ts FROM ws_search_history "
...
async def suggest(q: str = "", limit: int = 8):
    ...
    try:
        rows = con.execute(
            "SELECT DISTINCT query FROM ws_search_history "
            "WHERE query LIKE ? ORDER BY ts DESC LIMIT ?",
            (f"{q}%", limit),
        ).fetchall()
        suggestions = [r["query"] for r in rows]
    except Exception:
