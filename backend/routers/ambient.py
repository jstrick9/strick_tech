...
async def ambient_scan(req: Request):
    ...
    try:
        for s in suggestions[:50]:  # cap at 50 per scan
            try:
                con.execute("""INSERT INTO ambient_suggestions
                               (category,title,description,file_path,line_no,severity,action_type,action_data)
                               VALUES (?,?,?,?,?,?,?,?)""",
                            (s["category"], s["title"][:200], s.get("description","")[:500],
                             s.get("file_path",""), s.get("line_no",0), s.get("severity","info"),
                             s.get("action_type","suggestion"), s.get("action_data","{}")))
                saved += 1
            except Exception:
                pass
        con.commit()
...
async def project_health():
    ...
    try:
        ...
        try:
            ...
            avg_complex  = con.execute("SELECT AVG(complexity) FROM code_symbols WHERE symbol_type='function'").fetchone()[0] or 1
        finally:
            con.close()
        if total_syms > 0:
    ...
    try:
        ...
        try:
            ...
            with_docs = con.execute("SELECT COUNT(*) FROM code_symbols WHERE symbol_type='function' AND length(docstring)>10").fetchone()[0]
        finally:
            con.close()
        if total_fns > 0:
    ...
    con = get_conn()
    try:
        con.execute("""INSERT INTO health_snapshots
                       (overall_score,complexity_score,security_score,debt_score,docs_score,deps_score,details_json)
                       VALUES (?,?,?,?,?,?,?)""",
                    (overall, scores["complexity"], scores["security"],
                     scores["debt"], scores["docs"], scores["deps"], json.dumps(details)))
        con.commit()
    finally:
        con.close()
...
async def _execute_background_task(task_id: str, name: str, prompt: str, agent_id: str):
    ...
    try:
        result = await llm_svc.complete(
            ...
            agent_id=agent_id, max_tokens=2000, inject_steering=False
        )
...
def list_background_tasks(status: str = "", limit: int = 20):
    ...
    try:
        ...
        sql = "SELECT * FROM background_tasks" + (f" WHERE {' AND '.join(where)}" if where else "") + " ORDER BY created_at DESC LIMIT ?"
