...
async def ai_schema_designer(req: Request):
    ...
    messages = [
        {"role": "system", "content":
            "You are a SQLite database schema expert. "
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if r.status_code == 200:
                return {"ok": True, "table": table, "rows": r.json(),
                        "count": len(r.json())}
...
async def supabase_insert(req: Request):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            return {"ok": r.status_code in (200, 201), "data": r.json() if r.status_code in (200,201) else r.text[:200]}
    except Exception as e:
        return {"ok": False, "error": str(e)}
