...
FUSION_PRESETS = {
    ...
    "budget": {
        "panel":   ["google/gemini-2.0-flash-exp:free", "meta-llama/llama-3.3-70b-instruct:free", "qwen/qwen-2.5-72b-instruct:free"],
...
async def _call_model(model: str, messages: list, max_tokens: int = 1024,
                      temperature: float = 0.7) -> dict:
    """Call a single model and return its response."""
    import httpx
    key = os.getenv("OPENROUTER_API_KEY", "")
    if not key:
        return {"model": model, "text": f"[Stub: {model} — set OPENROUTER_API_KEY]",
                "tokens": 0, "latency_ms": 0, "error": False}
    ...
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            if r.status_code == 200:
                ...
                return {"model": model, "text": f"[Error {r.status_code}: {r.text[:200]}]",
                        "tokens": 0, "latency_ms": latency_ms, "error": True}
    except Exception as ex:
        return {"model": model, "text": f"[Error: {ex}]",
                "tokens": 0, "latency_ms": int((time.perf_counter() - t0) * 1000), "error": True}


async def _judge_responses(panel_responses: list[dict], original_prompt: str,
                           judge_model: str) -> str:
...
Synthesize into one optimal answer:"""
    ...
    result = await _call_model(judge_model, [{"role": "user", "content": judge_prompt}],
                               max_tokens=2000)
...
def _save_run_history(run_type: str, preset: str, prompt: str,
                      panel: list, synthesis: str, total_ms: int, total_tokens: int):
...
def get_history(limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            """)
            rows = con.execute(
                "SELECT * FROM fusion_history ORDER BY id DESC LIMIT ?",
                (min(limit, 100),)
            ).fetchall()
...
async def fusion_simple(req: Request):
    ...
    _save_run_history("fusion_simple", preset, prompt, panel, synthesis, total_ms,
                      sum(r.get("tokens", 0) for r in responses))

    return {
        "ok":        True,
        "synthesis": synthesis,
        "panel":     [{"model": r["model"], "preview": r["text"][:200],
                       "tokens": r["tokens"], "error": r.get("error", False)} for r in responses],
        "preset":    preset,
        "total_ms":  total_ms,
    }
...
def _classify_task(prompt: str) -> str:
    ...
    if any(w in p for w in ["tldr", "tl;dr", "tl dr", "brief summary", "short summary",
                             "give me a summary", "can you summarize"]):
        return "summary"
    if any(w in p for w in ["def ", "class ", "function", "import ", "```python", "```js",
                             "code", "bug", "debug", "error", "traceback", "fix this",
                             "write a function", "write a class", "implement", "refactor"]):
        return "code"
    if any(w in p for w in ["research", "explain", "what is", "who is", "history of",
                             "tell me about", "overview", "compare", "how does", "what are"]):
        return "research"
    if any(w in p for w in ["write a story", "poem", "creative", "imagine", "describe",
                             "narrative", "fiction"]):
        return "creative"
    if any(w in p for w in ["analyze", "analysis", "pros and cons", "evaluate", "assess",
                             "review", "critique", "strengths", "weaknesses"]):
        return "analysis"
    if any(w in p for w in ["calculate", "solve", "math", "equation", "proof",
                             "formula", "algebra", "compute", "integral", "derivative"]):
        return "math"
    if any(w in p for w in ["translate", "in french", "in spanish", "in chinese",
                             "in german", "in japanese", "in arabic"]):
        return "translation"
    if any(w in p for w in ["summarize", "summary", "tldr", "brief", "short version",
...
Synthesize all results into a complete, unified answer to the original task."""
        ...
        _save_run_history("subagent", "N/A", task, [orchestrator, worker],
                          final.get("text", ""), total_ms, total_tokens)
