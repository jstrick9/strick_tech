...
def _db_update_session(session_id: str, status: str,
                        steps: list, result: str, error: str = "") -> None:
    from ..services.memory_db import get_conn
    con = get_conn()
    try:
        con.execute(
...
async def run_browser_task(req: Request):
    ...
    try:
        con.execute(
            "INSERT INTO browser_sessions(id,url,task,status) VALUES (?,?,?,'pending')",
            (session_id, start_url, task)
        )
        con.commit()
    finally:
        con.close()
        ...
        try:
            async with async_playwright() as p:
                ...
                ctx = await browser.new_context(
                    ...
                    user_agent="Mozilla/5.0 AgenticOS/6.0 BrowserAgent"
                )
...
Return ONLY valid JSON."""
                    ...
                    ai_result = await llm_svc.complete(
                        ...
                        agent_id="browser", max_tokens=250, temperature=0.2,
                    ...
                    if not action_obj or action_obj.get("action") == "done":
                        done_step = {"action": "done", "reason": action_obj.get("reason", "Task complete"), "status": "done", "step_no": step_no}
...
Be realistic, specific, and complete the task fully."""
    ...
    result = await llm_svc.complete(
        ...
        agent_id="browser", max_tokens=800, inject_steering=False
    )
