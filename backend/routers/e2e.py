...
import asyncio, base64, datetime, json, logging, time, uuid
...
async def e2e_run(req: Request):
    ...
    url    = body.get("url") or (
        f"http://localhost:{_get_port()}/preview/mobile/index.html" if target == "mobile"
...
async def e2e_autofix(req: Request):
    ...
    for i in range(1, max_iters + 1):
        run_result = await e2e_run(Request(scope={"type":"http"},
                                           receive=_make_receive({"target": target})))
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            ctx       = await browser.new_context(
                viewport={"width": 390 if is_mobile else 1280,
                           "height": 844 if is_mobile else 800},
                is_mobile=is_mobile,
                has_touch=is_mobile,
            ...
            try:
                cta = page.locator(
                    "button, [role=button], a[href], [class*=btn], [id*=cta]"
                ).first
...
async def _heuristic_trace(url: str, target: str) -> list[dict]:
    ...
    steps.append(chk("file_exists",   path.exists(),       f"{path.name} found" if path.exists() else "No index.html found",
    ...
    if html:
        ...
        steps.append(chk("has_cta",        any(t in html.lower() for t in ["button","btn","cta","get-started","onclick"]),
                          "CTA element detected",  "Add a <button> or link"))
...
async def accessibility_audit(req: Request):
    ...
    try:
        ...
        body = {}
    url  = body.get("url", f"http://localhost:8787/preview/index.html")
    
    try:
        from playwright.async_api import async_playwright
    except ImportError:
        return {"ok": False, "error": "Playwright not installed", "mock_issues": [
            {"id":"color-contrast","description":"Text must have sufficient color contrast","impact":"serious","count":3},
            {"id":"aria-labels","description":"Form elements must have labels","impact":"moderate","count":1},
