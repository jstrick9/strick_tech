...
def close_tab(tab_id: str):
    """Execute or process close tab operation."""
    tabs = _get_tabs()
...
def activate_tab(tab_id: str):
    """Execute or process activate tab operation."""
    tabs = _get_tabs()
    if tab_id not in tabs:
        return {'ok': False, 'error': 'Tab not found'}
    for t in tabs.values():
...
def refresh_tab(tab_id: str):
    """Execute or process refresh tab operation."""
    tabs = _get_tabs()
    if tab_id not in tabs:
        return {'ok': False, 'error': 'Tab not found'}
...
def refresh_all():
    """Execute or process refresh all operation."""
    ts = time.time()
    for t in _get_tabs().values():
        t['last_refresh'] = ts
