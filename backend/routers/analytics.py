...
def dashboard(days: int = 30):
    ...
    try:
        ...
        # ── Agents ─────────────────────────────────────────────────────
        agents = con.execute(
            "SELECT id, name, status, avatar, color FROM agents ORDER BY name"
        ).fetchall()
        agent_messages = con.execute(
            "SELECT agent, COUNT(*) as messages FROM chat_log GROUP BY agent"
        ).fetchall()
        ...
        e2e_pass  = con.execute(
            "SELECT COUNT(DISTINCT run_id) FROM e2e_traces WHERE status='pass'"
        ).fetchone()[0]
    ...
    return {
        ...
        "agents": [
            {**dict(a), "messages": agent_msg_map.get(a["id"], 0)}
            for a in agents
        ],
...
def get_kpis():
    ...
    con = get_conn()
    try:
        cost_row     = con.execute("SELECT COALESCE(SUM(tokens),0) as t, COALESCE(SUM(cost),0) as c FROM chat_log").fetchone()
...
def export_dashboard(fmt: str = "csv", days: int = 30):
    ...
    for r in cost_rows:
        writer.writerow([r["day"], r["agent"], r["tokens"],
                         round(_safe_float(r["cost"]), 6), r["messages"]])
    output.seek(0)
