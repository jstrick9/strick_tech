...
BUILTIN_SERVERS = [
    {"server_id":"srv_filesystem","name":"File System","description":"Read, write, list, delete files within the workspace","server_type":"builtin","endpoint":"internal://filesystem","auth_type":"none","tools_schema":json.dumps([{"name":"fs.read","desc":"Read a file","args":["path"]},{"name":"fs.write","desc":"Write a file","args":["path","content"]},{"name":"fs.list","desc":"List directory","args":["path"]},{"name":"fs.delete","desc":"Delete a file","args":["path"]}]),"status":"active","rate_limit_rpm":120,"rate_limit_day":5000,"tags":"files,storage"},
    {"server_id":"srv_web_search","name":"Web Search","description":"DuckDuckGo and web search with citation support","server_type":"builtin","endpoint":"internal://search","auth_type":"none","tools_schema":json.dumps([{"name":"search.web","desc":"Search the web","args":["query","limit"]}]),"status":"active","rate_limit_rpm":20,"rate_limit_day":500,"tags":"search,web,research"},
    {"server_id":"srv_code_exec","name":"Code Executor","description":"Run Python code snippets in a sandboxed environment","server_type":"builtin","endpoint":"internal://code","auth_type":"none","tools_schema":json.dumps([{"name":"code.run","desc":"Execute Python code","args":["code"]}]),"status":"active","rate_limit_rpm":30,"rate_limit_day":500,"tags":"code,python,execution"},
    {"server_id":"srv_memory","name":"Memory Store","description":"Read and write to the agent memory knowledge base","server_type":"builtin","endpoint":"internal://memory","auth_type":"none","tools_schema":json.dumps([{"name":"memory.add","desc":"Add to memory","args":["content","tags"]},{"name":"memory.search","desc":"Search memory","args":["query","limit"]}]),"status":"active","rate_limit_rpm":60,"rate_limit_day":2000,"tags":"memory,knowledge"},
    {"server_id":"srv_http","name":"HTTP Client","description":"Make outbound HTTP GET/POST requests to external APIs","server_type":"builtin","endpoint":"internal://http","auth_type":"none","tools_schema":json.dumps([{"name":"http.get","desc":"HTTP GET","args":["url","headers"]},{"name":"http.post","desc":"HTTP POST","args":["url","body","headers"]}]),"status":"active","rate_limit_rpm":30,"rate_limit_day":1000,"tags":"http,api,external"},
    {"server_id":"srv_connectors","name":"Enterprise Connectors","description":"Slack, Jira, Google Workspace and other enterprise system connectors","server_type":"connector","endpoint":"internal://connectors","auth_type":"api_key","tools_schema":json.dumps([{"name":"slack.message","desc":"Send Slack message","args":["channel","text"]},{"name":"jira.create","desc":"Create Jira issue","args":["project","title","description"]},{"name":"gdocs.create","desc":"Create Google Doc","args":["title","content"]}]),"status":"active","rate_limit_rpm":20,"rate_limit_day":500,"tags":"enterprise,slack,jira,google"},
]

DEFAULT_POLICIES = [
    {"policy_id":"pol_allow_builtin","name":"Allow all built-in tools","description":"Agents can use all built-in MCP servers","agent_id":"*","server_id":"srv_filesystem,srv_web_search,srv_memory,srv_http,srv_code_exec","tool_pattern":"*","action":"allow","priority":100},
    {"policy_id":"pol_block_delete_prod","name":"Block file delete in prod","description":"No agent may delete files without elevated identity","agent_id":"*","server_id":"srv_filesystem","tool_pattern":"fs.delete","action":"require_hitl","priority":50},
    {"policy_id":"pol_rate_external","name":"Rate-limit external HTTP","description":"Cap HTTP calls per agent to 30/min","agent_id":"*","server_id":"srv_http","tool_pattern":"http.*","action":"allow","priority":90},
    {"policy_id":"pol_connector_elevated","name":"Connectors need elevated identity","description":"Enterprise connectors require elevated authority","agent_id":"orchestrator,brain","server_id":"srv_connectors","tool_pattern":"*","action":"allow","priority":80},
]
...
def _ensure_schema():
    ...
    try:
        ...
        for srv in BUILTIN_SERVERS:
            existing = con.execute("SELECT server_id FROM mcp_servers WHERE server_id=?", (srv["server_id"],)).fetchone()
            if not existing:
                con.execute("""INSERT INTO mcp_servers (server_id,name,description,server_type,endpoint,auth_type,tools_schema,status,rate_limit_rpm,rate_limit_day,tags,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (srv["server_id"],srv["name"],srv["description"],srv["server_type"],srv["endpoint"],srv["auth_type"],srv["tools_schema"],srv["status"],srv["rate_limit_rpm"],srv["rate_limit_day"],srv["tags"],now,now))
        for pol in DEFAULT_POLICIES:
            existing = con.execute("SELECT policy_id FROM mcp_gateway_policies WHERE policy_id=?", (pol["policy_id"],)).fetchone()
            if not existing:
                con.execute("""INSERT INTO mcp_gateway_policies (policy_id,name,description,agent_id,server_id,tool_pattern,action,priority,enabled,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,1,?,?)""",
                    (pol["policy_id"],pol["name"],pol["description"],pol["agent_id"],pol["server_id"],pol["tool_pattern"],pol["action"],pol["priority"],now,now))
        con.commit()
    finally:
        con.close()
...
def _evaluate_policy(agent_id: str, server_id: str, tool_name: str) -> tuple[str, str]:
    ...
    try:
        policies = con.execute(
            "SELECT * FROM mcp_gateway_policies WHERE enabled=1 ORDER BY priority ASC"
        ).fetchall()
...
def _check_rate_limit(agent_id: str, server_id: str, limit_rpm: int) -> bool:
    ...
    con = _get_conn()
    try:
        row = con.execute("SELECT count, window_start FROM mcp_rate_buckets WHERE bucket_key=?", (bucket_key,)).fetchone()
        if row:
            if row["window_start"] == now_minute:
                ...
                # New minute window
                con.execute("UPDATE mcp_rate_buckets SET count=1, window_start=? WHERE bucket_key=?", (now_minute, bucket_key))
        else:
            con.execute("INSERT INTO mcp_rate_buckets (bucket_key,count,window_start) VALUES (?,1,?)", (bucket_key, now_minute))
        con.commit()
        return True
    finally:
        con.close()
...
# ── Core Gateway call ──────────────────────────────────────────────────────────
async def gateway_call(agent_id: str, server_id: str, tool_name: str,
                        args: dict, context: dict | None = None) -> dict:
    """
    ...
    if policy_decision == "deny":
        _record_call(call_id, server_id, tool_name, agent_id, args, "blocked",
                     policy_decision, 0, 0, 0, f"Blocked by policy: {policy_name}")
        return {"ok": False, "error": f"Tool call blocked by policy '{policy_name}'",
                "policy_decision": "deny", "call_id": call_id}
    ...
    if srv_row:
        ...
        if not _check_rate_limit(agent_id, server_id, srv.get("rate_limit_rpm", 60)):
            _record_call(call_id, server_id, tool_name, agent_id, args, "rate_limited",
                         policy_decision, 0, 0, 0, "Rate limit exceeded")
            return {"ok": False, "error": "Rate limit exceeded for this agent/server combination",
                    "policy_decision": "rate_limited", "call_id": call_id}
    ...
    if policy_decision == "require_hitl":
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5) as client:
                await client.post(f"http://127.0.0.1:{port}/api/hitl/interrupt",
                    json={"action_type": tool_name, "action_summary": f"MCP tool call: {tool_name}({json.dumps(args)[:100]})",
                          "risk_level": "high", "confidence": 0.7, "agent_id": agent_id,
                          "action_data": {"server_id": server_id, "tool": tool_name, "args": args}})
        except Exception:
            pass
        _record_call(call_id, server_id, tool_name, agent_id, args, "hitl_pending",
                     policy_decision, 0, 0, 0, "")
        return {"ok": False, "pending": True, "call_id": call_id,
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(f"http://127.0.0.1:{port}/api/mcp/call",
                json={"tool": tool_name, "args": args, "agent_id": agent_id})
            result = resp.json()
    except Exception as e:
        err = str(e)[:300]
        _record_call(call_id, server_id, tool_name, agent_id, args, "error",
                     policy_decision, _epoch()*1000-t0, 0, 0, err)
        return {"ok": False, "error": err, "call_id": call_id}

    duration_ms = _epoch()*1000 - t0
    status      = "ok" if result.get("ok") else "error"

    _record_call(call_id, server_id, tool_name, agent_id, args, status,
                 policy_decision, duration_ms, 0, 0, result.get("error",""))

    # 5. Audit chain entry
    try:
        from ..routers.audit_log import append_entry
        append_entry(agent_id, agent_id.title(), "mcp_tool_call",
                     f"[{server_id}] {tool_name}({str(args)[:80]})",
                     reasoning=f"Policy: {policy_name}",
                     authority="agent", risk_level="low",
                     outcome="success" if result.get("ok") else "failure",
                     metadata={"call_id": call_id, "server_id": server_id,
                               "tool": tool_name, "duration_ms": duration_ms})
    except Exception:
        pass

    return {**result, "call_id": call_id, "policy_decision": policy_decision,
            "gateway_duration_ms": duration_ms}


def _record_call(call_id, server_id, tool_name, agent_id, args, status,
                  policy_decision, duration_ms, tokens, cost, error):
    args_hash = hashlib.sha256(json.dumps(args, sort_keys=True, default=str).encode()).hexdigest()[:16]
    con = _get_conn()
    try:
        con.execute("""INSERT OR IGNORE INTO mcp_gateway_calls
            (call_id,server_id,tool_name,agent_id,args_hash,status,policy_decision,duration_ms,tokens_used,cost,error,created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
            (call_id, server_id, tool_name, agent_id, args_hash, status,
             policy_decision, duration_ms, tokens, cost, error[:500], _now()))
        con.commit()
    finally:
        con.close()
...
def list_servers(status: str = ""):
    ...
    for r in rows:
        d = dict(r)
        try: d["tools_schema"] = json.loads(d["tools_schema"])
        except: pass
        try: d["auth_config"] = {"type": d["auth_type"], "configured": bool(json.loads(d.get("auth_config","{}")).get("key"))}
        except: pass
        servers.append(d)
...
async def register_server(req: Request):
    ...
    con = _get_conn()
    try:
        con.execute("""INSERT INTO mcp_servers (server_id,name,description,server_type,endpoint,auth_type,auth_config,tools_schema,status,rate_limit_rpm,rate_limit_day,tags,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (server_id, name[:100],
...
async def toggle_server(server_id: str, req: Request):
    ...
    con = _get_conn()
    try:
        con.execute("UPDATE mcp_servers SET status=?, updated_at=? WHERE server_id=?",
...
async def create_policy(req: Request):
    ...
    try:
        con.execute("""INSERT INTO mcp_gateway_policies (policy_id,name,description,agent_id,server_id,tool_pattern,action,priority,enabled,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,1,?,?)""",
            (pol_id, name[:100], (body.get("description") or "")[:500],
             (body.get("agent_id") or "*")[:100], (body.get("server_id") or "*")[:100],
             (body.get("tool_pattern") or "*")[:100], action,
             int(body.get("priority") or 100), now, now))
        con.commit()
    finally:
        con.close()
...
async def simulate_policy(req: Request):
    ...
    if srv and srv["status"] == "disabled":
        ...
        trace.append({
            ...
            "agent_match": True, "server_match": True, "tool_match": True,
        })
    else:
        for pol in policies:
            pol_d = dict(pol)
    ...
    for i, a in enumerate(pols):
        for j, b in enumerate(pols):
            ...
            def overlaps(pat_a, pat_b):
                return (fnmatch.fnmatch(pat_a, pat_b) or
                        fnmatch.fnmatch(pat_b, pat_a) or
                        pat_a == "*" or pat_b == "*")

            agent_overlap  = a["agent_id"] == "*" or b["agent_id"] == "*" or \
                             any(ag in b["agent_id"].split(",") for ag in a["agent_id"].split(","))
            server_overlap = a["server_id"] == "*" or b["server_id"] == "*" or \
                             any(sv in b["server_id"].split(",") for sv in a["server_id"].split(","))
            ...
            # Exact duplicate
            if (a["agent_id"] == b["agent_id"] and a["server_id"] == b["server_id"] and
                    a["tool_pattern"] == b["tool_pattern"] and a["action"] == b["action"]):
                conflicts.append({
                    "type": "duplicate",
                    "severity": "warning",
            ...
            # Conflicting action (same scope, different action)
                ...
                conflicts.append({
                    "type": "conflict",
                    "severity": "error",
                    "policy_a": {"id": a["policy_id"], "name": a["name"], "action": a["action"], "priority": a["priority"]},
                    "policy_b": {"id": b["policy_id"], "name": b["name"], "action": b["action"], "priority": b["priority"]},
...
async def create_policy_from_template(req: Request):
    ...
    con = _get_conn()
    try:
        con.execute("""INSERT INTO mcp_gateway_policies
            (policy_id,name,description,agent_id,server_id,tool_pattern,action,priority,conditions,enabled,created_at,updated_at)
            VALUES (?,?,?,?,?,?,?,?,'{}',1,?,?)""",
            (pol_id, name[:100], description[:500], agent_id[:100], server_id[:100],
             tool_pattern[:100], action, priority, now, now))
        con.commit()
    finally:
        con.close()
...
def toggle_policy(policy_id: str):
    ...
    try:
        ...
        con.execute("UPDATE mcp_gateway_policies SET enabled=?, updated_at=? WHERE policy_id=?",
...
async def update_policy(policy_id: str, req: Request):
    ...
    allowed = {"name", "description", "action", "agent_id", "server_id",
               "tool_pattern", "priority", "conditions"}
    updates = {}
    for k, v in body.items():
        if k not in allowed:
            continue
        if k == "action" and v not in ("allow", "deny", "require_hitl"):
            return JSONResponse({"ok": False, "error": f"action must be allow|deny|require_hitl, got {v}"}, status_code=400)
        if k == "priority":
            try:
                updates[k] = int(v)
    ...
    try:
        ...
        if not row:
            return JSONResponse({"ok": False, "error": "Policy not found"}, status_code=404)
        sets = ", ".join(f"{k}=?" for k in updates)
        con.execute(f"UPDATE mcp_gateway_policies SET {sets} WHERE policy_id=?",
                    list(updates.values()) + [policy_id])
        con.commit()
    finally:
        con.close()
...
def list_calls(agent_id: str="", server_id: str="", status: str="", limit: int=50):
    ...
    if server_id: where.append("server_id=?"); params.append(server_id)
...
def gateway_stats():
    ...
    try:
        ...
        by_agent = con.execute("SELECT agent_id,COUNT(*) cnt FROM mcp_gateway_calls GROUP BY agent_id ORDER BY cnt DESC LIMIT 10").fetchall()
        by_tool  = con.execute("SELECT tool_name,COUNT(*) cnt FROM mcp_gateway_calls GROUP BY tool_name ORDER BY cnt DESC LIMIT 10").fetchall()
        blocked  = con.execute("SELECT COUNT(*) FROM mcp_gateway_calls WHERE policy_decision IN ('deny','rate_limited')").fetchone()[0]
...
def get_agent_card(agent_id: str):
    ...
    try:
        ...
        identity = con.execute("SELECT public_key, key_version, authority_level FROM agent_identities WHERE agent_id=?", (agent_id,)).fetchone()
        perms = con.execute("SELECT action FROM agent_permissions WHERE agent_id=?", (agent_id,)).fetchall()
        tools = con.execute("SELECT DISTINCT tool_name FROM mcp_gateway_calls WHERE agent_id=? ORDER BY tool_name", (agent_id,)).fetchall()
    finally:
        con.close()
