...
def _record_frame(run_id: str, frame_no: int, node_id: str, node_type: str,
                  node_label: str, event_type: str, input_ctx: dict,
                  output: str, error: str, duration_ms: int):
    from ..services.memory_db import get_conn
    con = get_conn()
    try:
        con.execute(
...
async def recorded_run(wf_id: str, req: Request):
    ...
    async def _stream():
        ...
        while queue:
            ...
            # Emit node_start
            frame_no += 1
            start_ev = {"type":"node_start","run_id":run_id,"frame":frame_no,
                        "node_id":nid,"node_type":node_type,"label":label,
                        "context_snapshot":{"input":context["input"],
                                            "prev_output":context["prev_output"][:200]}}
            yield f"data: {json.dumps(start_ev)}\n\n"
            _record_frame(run_id, frame_no, nid, node_type, label, "node_start",
            ...
            if node_type == "trigger":
                ...
                prompt = (prompt
                          ...
                          .replace("{{prev_output}}", context["prev_output"]))
                try:
                ...
                if url:
                    ...
                    try:
                        ...
                        error = str(ex); output = f"[webhook error] {ex}"
                ...
                if code:
                    try:
                        ...
                        error = str(ex); output = f"[code error] {ex}"
                else:
                    output = "no code"
            ...
            frame_no += 1
            _record_frame(run_id, frame_no, nid, node_type, label, "node_output",
                          {"input":context["input"],"prev_output":context["prev_output"][:500]},
                          output, error, dur_ms)

            done_ev = {"type":"node_output","run_id":run_id,"frame":frame_no,
                       "node_id":nid,"node_type":node_type,"label":label,
                       "output":output[:300],"error":error,"duration_ms":dur_ms,
...
def get_run(run_id: str):
    ...
    try:
        ...
        frames = con.execute(
            "SELECT * FROM workflow_run_frames WHERE run_id=? ORDER BY frame_no",
            (run_id,)
        ).fetchall()
    finally:
        con.close()
...
def get_frame(run_id: str, frame_no: int):
    ...
    try:
        row = con.execute(
            "SELECT * FROM workflow_run_frames WHERE run_id=? AND frame_no=?",
            (run_id, frame_no)
        ).fetchone()
    finally:
        con.close()
...
async def rerun_from_frame(run_id: str, frame_no: int, req: Request):
    ...
    async def _stream():
        ...
        while queue:
            ...
            if node["type"] == "agent":
                prompt = cfg.get("prompt","{{input}}")
                prompt = prompt.replace("{{input}}",context.get("input","")).replace("{{prev_output}}",context.get("prev_output",""))
                try:
                    result = await llm_svc.complete([{"role":"user","content":prompt}], agent_id=cfg.get("agent_id","builder"), max_tokens=1024, inject_steering=False)
                    output = result.get("text","")
                    context["prev_output"] = output
                except Exception as ex:
                    error = str(ex)
            ...
            dur_ms = int((time.perf_counter() - t0) * 1000)
            fn += 1
            _record_frame(new_run_id, fn, nid, node["type"], label, "node_output",
                          context, output, error, dur_ms)
...
def diff_runs(run_id_a: str, run_id_b: str):
    ...
    con = get_conn()
    try:
        a_frames = {r["node_id"]: dict(r) for r in con.execute(
            "SELECT * FROM workflow_run_frames WHERE run_id=? AND event_type='node_output' ORDER BY frame_no",
            (run_id_a,)).fetchall()}
        b_frames = {r["node_id"]: dict(r) for r in con.execute(
            "SELECT * FROM workflow_run_frames WHERE run_id=? AND event_type='node_output' ORDER BY frame_no",
            (run_id_b,)).fetchall()}
    finally:
        con.close()
