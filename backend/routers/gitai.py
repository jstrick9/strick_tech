...
def _git(args: list[str], cwd=None) -> tuple[str, str, int]:
    ...
    try:
        r = subprocess.run(
            ...
            cwd=str(cwd or ROOT), timeout=15
        )
...
- Return ONLY valid JSON."""
    ...
    result = await llm_svc.complete(
        ...
        agent_id="gitai", max_tokens=800, temperature=0.1, inject_steering=False
    )
    ...
    for cmd_info in commands:
        ...
        if not safe and not body.get("allow_unsafe", False):
            results.append({"cmd":" ".join(cmd),"skipped":True,"reason":"Unsafe command skipped (set allow_unsafe=true)"})
            continue
...
Return ONLY the commit message, no explanation."""
    ...
    result = await llm_svc.complete(
        ...
        agent_id="gitai", max_tokens=200, temperature=0.3, inject_steering=False
    )
...
Return only the markdown changelog section."""
    ...
    result = await llm_svc.complete(
        [{"role":"user","content":prompt}],
        agent_id="gitai", max_tokens=1500, temperature=0.3, inject_steering=False
    )
...
Return ONLY valid JSON."""
    ...
    result = await llm_svc.complete(
        [{"role":"user","content":prompt}],
        agent_id="gitai", max_tokens=2000, temperature=0.1, inject_steering=False
    )
    text = result.get("text","")
    m    = re.search(r'\{.*\}', text, re.DOTALL)
    audit_result = {}
...
SECURITY_RULES = [
    ...
    {"id":"A01","name":"Missing Auth Check","pattern":r"@app\.(get|post|put|delete)\(['\"](?!/api/auth).*['\"\)]\)\s*\nasync def [^(]+\([^)]*\):",
     "desc":"API endpoint may be missing authentication","severity":"high","category":"access_control"},

    # A02: Cryptographic Failures
    {"id":"A02-1","name":"Weak Hash (MD5)","pattern":r"md5\(|hashlib\.md5",
     "desc":"MD5 is cryptographically broken","severity":"high","category":"crypto"},
    {"id":"A02-2","name":"Weak Hash (SHA1)","pattern":r"sha1\(|hashlib\.sha1",
     "desc":"SHA1 is considered weak for security use","severity":"medium","category":"crypto"},
    {"id":"A02-3","name":"Hardcoded Secret","pattern":r'(?i)(password|secret|api_key|token)\s*=\s*["\'][^"\']{8,}["\']',
     "desc":"Hardcoded credentials detected","severity":"critical","category":"crypto"},

    # A03: Injection
    {"id":"A03-1","name":"SQL Injection Risk","pattern":r'(?i)(execute|cursor\.execute)\s*\(\s*f["\'].*{',
     "desc":"f-string SQL query may allow injection","severity":"critical","category":"injection"},
    {"id":"A03-2","name":"Shell Injection Risk","pattern":r'subprocess\.(run|Popen|call)\(.*shell=True',
     "desc":"shell=True with user input risks command injection","severity":"high","category":"injection"},
    {"id":"A03-3","name":"XSS Risk","pattern":r'innerHTML\s*[+]?=\s*(?!.*escHtml)[^;]+(?:user|input|param|query)',
     "desc":"innerHTML with unescaped user input may allow XSS","severity":"high","category":"injection"},

    # A04: Insecure Design
    {"id":"A04","name":"Debug Mode","pattern":r'debug\s*=\s*True|DEBUG\s*=\s*True',
     "desc":"Debug mode should not be enabled in production","severity":"medium","category":"design"},

    # A05: Security Misconfiguration
    {"id":"A05-1","name":"CORS Wildcard","pattern":r'allow_origins.*\[.*\"\*\"|CORSMiddleware.*origins.*\*',
     "desc":"Wildcard CORS allows any origin — restrict in production","severity":"medium","category":"misconfig"},
    {"id":"A05-2","name":"No Rate Limiting","pattern":r'@app\.(post|put)\(["\']',
     "desc":"Consider adding rate limiting to mutation endpoints","severity":"low","category":"misconfig"},

    # A07: Authentication Failures
    {"id":"A07","name":"Plain Text Password","pattern":r'password\s*=\s*request|password\s*=\s*body',
     "desc":"Ensure passwords are never stored or logged as plaintext","severity":"high","category":"auth"},

    # A09: Logging Failures
    {"id":"A09","name":"Sensitive Data in Logs","pattern":r'log\.(info|debug|warning)\(.*(?:password|token|secret|key)',
     "desc":"Sensitive data may be logged","severity":"medium","category":"logging"},

    # A10: SSRF
    {"id":"A10","name":"SSRF Risk","pattern":r'httpx\.get\(.*(?:url|endpoint|target)|requests\.get\(.*(?:url|endpoint)',
     "desc":"Dynamic URLs from user input may allow SSRF","severity":"high","category":"ssrf"},
]
...
async def security_scan(req: Request):
    ...
    score = max(0, 100 - by_sev.get("critical",0)*20 - by_sev.get("high",0)*10 - by_sev.get("medium",0)*5 - by_sev.get("low",0)*2)
...
async def scan_file(req: Request):
    ...
    score = max(0, 100 - sum(
        {"critical":20,"high":10,"medium":5,"low":2}.get(v["severity"],0) for v in vulns
    ))
