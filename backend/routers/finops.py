...
def _check_budget_caps(agent_id: str, cost: float, tokens: int, goal_id: str):
    ...
    try:
        ...
        for cap in caps:
            ...
            # Update cap counters
            con.execute(
                'UPDATE budget_caps SET current_usd=?, current_tok=? WHERE cap_id=?', (cur_cost, cur_tok, cap['cap_id'])
            )

            # Alert thresholds. Both dimensions a cap can enforce: a cap with
            # only limit_tokens used to deny calls via the pre-flight gate
            # (over_tok) while remaining INVISIBLE here — no alert, no
            # breached flag, nothing but agents that stopped with no trail.
            usd_pct = (cur_cost / cap['limit_usd']) if cap['limit_usd'] > 0 else None
            tok_pct = (cur_tok / cap['limit_tokens']) if cap['limit_tokens'] > 0 else None
            breached_now = (usd_pct is not None and usd_pct >= 1.0) or (
                tok_pct is not None and tok_pct >= 1.0
            )
            if breached_now:
                if not cap['breached']:
                    con.execute('UPDATE budget_caps SET breached=1, breached_at=? WHERE cap_id=?', (now, cap['cap_id']))
                    # pct_used carries the worst dimension's ratio and
                    # cost_at_alert the USD spend at that moment; the cap row
                    # itself holds both limits for interpretation.
                    worst_pct = max(p for p in (usd_pct, tok_pct) if p is not None)
                    con.execute(
                        """INSERT INTO cost_alerts
                        (cap_id,agent_id,alert_type,pct_used,cost_at_alert,limit_usd,created_at)
                        VALUES (?,?,'breach',?,?,?,?)""",
                        (cap['cap_id'], agent_id, worst_pct, cur_cost, cap['limit_usd'], now),
                    )
                    log.warning(
                        'Budget BREACH: %s %.0f%% ($%.4f/$%.4f, %d/%d tok)',
                        cap['name'], worst_pct * 100, cur_cost, cap['limit_usd'], cur_tok, cap['limit_tokens'],
                    )
            else:
                if cap['breached']:
                    ...
                    log.info(
                        ...
                        cap['name'], (usd_pct or 0) * 100, cur_cost, cap['limit_usd'],
                    )
                # Warnings stay USD-only: cost_alerts has no dimension
                # column, so a token warning and a USD warning would collide
                # in the per-cap hourly dedup below. Breaches don't collide
                # (one-shot per episode via the flag), so both dimensions
                # breach-alert.
                if usd_pct is not None and usd_pct >= 0.8:
                    ...
                    if not recent:
                        con.execute(
                            ...
                            (cap_id,agent_id,alert_type,pct_used,cost_at_alert,limit_usd,created_at)
                            VALUES (?,?,'warning',?,?,?,?)""",
                            (cap['cap_id'], agent_id, usd_pct, cur_cost, cap['limit_usd'], now),
                        )
...
def finops_dashboard():
    ...
    try:
        ...
        by_src = con.execute(
            'SELECT source_type, SUM(cost_usd) c, COUNT(*) n FROM cost_ledger GROUP BY source_type ORDER BY c DESC'
        ).fetchall()
        by_agent = con.execute(
            'SELECT agent_id, SUM(cost_usd) c, SUM(total_tokens) t, COUNT(*) n FROM cost_ledger GROUP BY agent_id ORDER BY c DESC LIMIT 10'
        ).fetchall()
        by_model = con.execute(
            "SELECT model, SUM(cost_usd) c, SUM(total_tokens) t FROM cost_ledger WHERE model!='' GROUP BY model ORDER BY c DESC LIMIT 8"
        ).fetchall()
        today = con.execute(
            "SELECT SUM(cost_usd) c FROM cost_ledger WHERE created_at > datetime('now','-1 day')"
        ).fetchone()
        this_hr = con.execute(
            "SELECT SUM(cost_usd) c FROM cost_ledger WHERE created_at > datetime('now','-1 hour')"
        ).fetchone()
        caps = con.execute('SELECT * FROM budget_caps WHERE enabled=1').fetchall()
...
@router.get('/ledger')
def ledger_entries(
    agent_id: str = '', source_type: str = '', goal_id: str = '', run_id: str = '', days: int = 7, limit: int = 100
):
    ...
    try:
        rows = con.execute(
            f'SELECT * FROM cost_ledger WHERE {" AND ".join(where)} ORDER BY created_at DESC LIMIT ?', params
        ).fetchall()
        total_cost = con.execute(
            f'SELECT SUM(cost_usd) FROM cost_ledger WHERE {" AND ".join(where)}', params[:-1]
        ).fetchone()[0]
    finally:
        con.close()
    return {'entries': [dict(r) for r in rows], 'count': len(rows), 'total_cost': round(total_cost or 0, 6)}
...
def delete_cap(cap_id: str):
    """Delete or remove specified cap."""
...
def list_alerts(resolved: bool = False, limit: int = 50):
    """Retrieve and return list alerts."""
    con = _get_conn()
    try:
        rows = con.execute(
...
def resolve_alert(alert_id: int):
    """Execute or process resolve alert operation."""
    con = _get_conn()
    try:
