...
def index_status():
    ...
    try:
        indexed_count = con.execute(
            "SELECT COUNT(*) FROM memory WHERE source LIKE 'obsidian:%'"
        ).fetchone()[0]
    except Exception:
        indexed_count = 0
    finally:
...
async def index_vault(req: Request):
    ...
    from ..services.memory_db import memory_add, memory_search_fts, get_conn, audit_log
    ...
    for note in notes:
        try:
            ...
            if not re_index:
                ...
                if any(r.get("source", "").startswith("obsidian:") and title in r.get("content", "")
                       for r in existing):
                    skipped += 1
                    continue
...
async def create_daily_note(req: Request):
    ...
    try:
        ...
        tasks_doing = con.execute(
            "SELECT title, agent FROM tasks WHERE status='doing' LIMIT 5"
        ).fetchall()
        chat_count  = con.execute(
            "SELECT COUNT(*) FROM chat_log WHERE date(created_at)=date('now')"
        ).fetchone()[0]
        cost_today  = con.execute(
            "SELECT SUM(cost) FROM chat_log WHERE date(created_at)=date('now')"
        ).fetchone()[0] or 0.0
        mem_today   = con.execute(
            "SELECT COUNT(*) FROM memory WHERE date(created_at)=date('now')"
        ).fetchone()[0]
    finally:
        con.close()
