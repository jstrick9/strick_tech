...
def scaffold(ws: Path, name: str, description: str, stage_names: list[str]) -> dict[str, Any]:
    ...
    (ws / 'CONTEXT.md').write_text(
        ...
        f'| Stage | Handles |\n|-------|---------|\n{routing}\n\n'
        f'## Routes\n'
        f'<!-- What should arrive here? One trigger phrase per bullet.\n'
        f'     The Inbox router reads THIS section to decide where a captured\n'
        f'     item is filed. With no bullets the workspace can only be reached\n'
        f'     by name, and every inbox sweep will leave items unfiled.\n'
        f'     Example:\n'
        f'       - weekly client report\n'
        f'       - invoice\n'
        f'     Prose in this section is ignored, so you can explain freely. -->\n\n'
        f'## Conventions\n'
        f'- One stage, one job.\n'
