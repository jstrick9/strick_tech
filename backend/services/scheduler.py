...
import asyncio, logging, os, json, datetime
...
def add_loop(
    ...
    try:
        ...
        _jobs[job_id] = {
            "id": job_id, "prompt": prompt, "interval_minutes": interval_minutes,
            "agent_id": agent_id, "target": target, "status": "running",
            "goal_id": goal_id, "max_runs": max_runs,
...
_BUILTIN_JOB_IDS = frozenset({
    "memory_index", "standup", "cost_digest", "status_cleanup"
})
...
async def _run_goal_loop(job_id: str, prompt: str, agent_id: str, target: str):
    ...
    try:
        ...
        from .memory_db import memory_add, get_conn, audit_log

        messages = [
            {"role": "system", "content":
                "You are an autonomous agent in Agentic OS. "
